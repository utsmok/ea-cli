...
Recent milestones (merged from current `changelog.md` and the snapshot in `.github/sync_mismatch.md`):
...
- 2025-09-03: Refactored `easy_access/db/relations.py` to remove Mock-aware branching and adjusted unit tests; deterministic `bulk_update` semantics implemented.
- 2025-09-03: Added unit tests for enrichment staleness and fetch-error handling (`tests/test_enrichment_staleness.py`).
- 2025-09-05: Continued Phase D test expansion: added concurrency/missing-course coverage for `fetch_and_parse_courses` and adjusted tests to match timezone handling in enrichment code.
