...
class Classification(BaseModel):
    ...
    allowed_usage_reasoning: (
        str  # add a 1 to 2 sentence explanation on why this allowed usage was chosen
    )
    copyright_status: CopyrightStatus = CopyrightStatus.OTHER
    copyright_classification_reason: (
        str  # add a 1 to 2 sentence explanation on why this copyright status was chosen
    )
    item_type: ItemType = ItemType.UNKNOWN
    item_type_classification_reason: (
        str  # add a 1 to 2 sentence explanation on why this item type was chosen
    )
    pdf_name: str
    ...
    author_names: list[
        str
    ]  # the name of the author(s) that created the item, if possible to determine
    ...
    source_url: list[
        str
    ]  # the source URL(s) for the item if included in the document itself
