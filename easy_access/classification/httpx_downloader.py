...
from easy_access.db.base import CopyrightItem  # init renamed
...
def load_cookies_from_file() -> httpx.Cookies:
    ...
    cookie_file_path: Path = (
        SETTINGS.dirs[DirSetting.SCRIPT_DATA].full / "cookies.secret"
    )
    ...
    try:
        with open(cookie_file_path, encoding="utf-8") as f:  # Specify read mode 'r'
        ...
        for cookie_obj in cookie_data:
            if (
                ...
                domain: str | None = cookie_obj.get("domain")
        ...
        logger.error(
            f"Failed to decode JSON from cookie file {cookie_file_path.name}: {e}"
        )
        raise
    except Exception as e:  # Catch other potential errors like file read issues
        logger.error(
            f"Failed to load or parse cookie file '{cookie_file_path.name}': {e}"
        )
        raise
...
class HttpxDownloader:
    ...
    def __init__(self, timeout: float = DEFAULT_HTTP_TIMEOUT) -> None:
        ...
        logger.info(
            f"HttpxDownloader client initialized with timeout {timeout}s and loaded cookies."
        )
    ...
    async def download_file(
        ...
        try:
            ...
            material_id_from_url: str = (
                initial_url.split("files/")[1].split("?")[0].strip("/")
            )
        except IndexError:
        ...
        canvas_base_domain: str = "https://utwente.instructure.com"
        canvas_user_id_for_download: str = (
            "66733"  # This is highly specific and needs configuration
        )
        ...
        logger.info(
            f"Attempting download for Canvas ID {material_id_from_url} from: {download_url}"
        )
        try:
            async with self.client.stream("GET", download_url) as response:
                ...
                if bytes_downloaded > 0:
                    logger.info(
                        f"Successfully downloaded {bytes_downloaded} bytes to {output_filepath.name} (Canvas ID: {material_id_from_url})"
                    )
                    return True, output_filepath, None
                else:  # pragma: no cover (empty file is unlikely if status is 200)
                    logger.warning(
                        f"Downloaded an empty file for Canvas ID {material_id_from_url} from {download_url}."
                    )
                    # Clean up empty file
            ...
            if output_filepath.exists():
                output_filepath.unlink(missing_ok=True)  # Cleanup
            return False, None, error_msg


async def main_download_all(
    ...
    try:
        ...
        urls_df: pl.DataFrame = (
            await get_urls_from_full_data()
        )  # This function logs and handles its own DB connection
        if urls_df.is_empty():
        ...
        logger.info(
            f"Starting bulk download of {len(urls_to_download_list)} files (max concurrent: {max_concurrent})..."
        )

        download_tasks: list[asyncio.Task[tuple[bool, Path | None, str | None]]] = []
        ...
        for url_str in urls_to_download_list:
            download_tasks.append(
                asyncio.create_task(_download_with_semaphore(url_str))
            )
        ...
        for i, result_item in enumerate(results):
            ...
            if isinstance(result_item, Exception):
                logger.warning(
                    f"Download task for {original_url} raised an exception: {result_item}"
                )
                failed_url_errors.append((original_url, str(result_item)))
            elif isinstance(
                result_item, tuple
            ):  # Expected (success, filepath, error_msg)
                ...
                if success and filepath:
                    ...
                    failed_url_errors.append(
                        (original_url, error_msg or "Unknown download error")
                    )
            else:  # Should not happen
                logger.error(
                    f"Unexpected result type from download task for {original_url}: {type(result_item)}"
                )
                failed_url_errors.append((original_url, "Unknown internal error"))

        logger.info(
            f"Download process complete. Success: {success_count}, Failed: {len(failed_url_errors)}"
        )
...
def get_already_downloaded_material_ids() -> list[str]:
    ...
    if not pdf_download_dir or not pdf_download_dir.exists:
        logger.warning(
            "PDF download directory not configured or found. Cannot get downloaded IDs."
        )
        return []

    existing_pdf_filenames: list[str] = [
        file.name for file in pdf_download_dir.files if file.extension == ".pdf"
    ]
    ...
    valid_material_ids = [
        mid for mid in material_ids_on_disk if mid.strip()
    ]  # Basic check for non-empty
    logger.info(
        f"Found {len(valid_material_ids)} material IDs from files in download directory."
    )
...
async def get_urls_from_full_data() -> pl.DataFrame:
    ...
    try:
        ...
        items_with_urls_qs = (
            CopyrightItem.filter(
                url__isnull=False, status__not=Status.DELETED.value
            ).exclude(  # Use enum value
                url__in=["", "-"]
            )  # Exclude empty or placeholder URLs
        )
        all_relevant_items_list = await items_with_urls_qs.values(
            "material_id", "url", "filename"
        )

        logger.info(
            f"Fetched {len(all_relevant_items_list)} relevant items with URLs from DB (total items: {full_item_count})."
        )
        if not all_relevant_items_list:
            return pl.DataFrame()  # Return empty if no relevant items

        all_items_df = pl.from_dicts(
            all_relevant_items_list,
            schema={"material_id": pl.Int64, "url": pl.Utf8, "filename": pl.Utf8},
        )  # Specify schema
        ...
        material_ids_already_on_disk_str: list[str] = (
            get_already_downloaded_material_ids()
        )
        # Convert to int if material_id in DB is int for comparison
        material_ids_on_disk_int: Set[int] = {
            int(mid) for mid in material_ids_already_on_disk_str if mid.isdigit()
        }
        ...
        final_urls_df = items_to_download_df.select(
            ["url", "material_id", "filename"]
        ).unique(subset=["url"], maintain_order=True)

        downloaded_count = len(material_ids_on_disk_int)
        total_relevant_url_items = len(
            all_items_df
        )  # Count before filtering out downloaded
...
async def replace_canvas_id_with_material_id() -> None:
    ...
    try:
        ...
        if not download_dir.exists:
            logger.warning(
                f"PDF download directory {download_dir.full} not found. Cannot rename files."
            )
            return
        ...
        logger.info(
            f"Starting filename normalization for {len(all_files_in_download_dir)} files in {download_dir.full}..."
        )
        for item_map in all_items_db_info:
            ...
            try:
                ...
                canvas_id_from_url: str = (
                    url.split("files/")[1].split("?")[0].split("/")[0].strip("/")
                )
                if not canvas_id_from_url.isdigit():  # Ensure extracted ID is numeric
                    logger.debug(
                        f"Non-numeric Canvas ID '{canvas_id_from_url}' from URL '{url}'. Skipping."
                    )
                    continue
            except IndexError:
            ...
            for file_obj in all_files_in_download_dir:
                if file_obj.name.startswith(canvas_id_from_url):
                    ...
                    rest_of_filename = file_obj.name[
                        len(canvas_id_from_url) :
                    ]  # e.g., "_some_suffix.pdf" or ".pdf"
                    ...
                    if file_obj.name != new_filename:
                        try:
                            file_obj.rename(
                                new_filename
                            )  # File.rename handles moving to new name
                            logger.info(f"Renamed: {file_obj.name} -> {new_filename}")
                            renamed_count += 1
                        except Exception as e_rename:
                            logger.error(
                                f"Error renaming file {file_obj.name} to {new_filename}: {e_rename}"
                            )

        logger.info(f"Filename normalization complete. Renamed {renamed_count} files.")
    except Exception as e:
        logger.error(
            f"An error occurred during replace_canvas_id_with_material_id: {e}"
        )
        logger.debug(traceback.format_exc())
    finally:
        await Tortoise.close_connections()
...
async def download_pdfs_example_usage(
    ...
    """
    logger.info(
        f"Starting PDF download example with max {max_concurrent_downloads} concurrent downloads."
    )
    # Ensure Tortoise is initialized if DB operations are needed before/during download prep
    await init_tortoise_orm()

    downloaded_paths, failed_downloads = await main_download_all(
