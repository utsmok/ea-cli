...
from itertools import (
    batched,  # Python 3.12+ feature, ensure compatibility or provide alternative
)
from pathlib import Path
from typing import (  # Used Coroutine instead of CoroutineType
    Any,
)
...
from easy_access.db.ingest import (
    load_pdfs,  # To ensure PDFs are in DB before processing
)
...
PDF_PROCESSING_TIMEOUT: int = (
    30  # Timeout in seconds for potentially long operations like text extraction
)
...
async def batch_extract_pdf_text(
    ...
    max_pages: int | None = PDF_TEXT_MAX_PAGES,
    str_limit: int | None = PDF_TEXT_STR_LIMIT,
) -> list[PDF]:
    """
    ...
    valid_pdfs_for_extraction: list[PDF] = [
        ...
        if p.path.exists()
        and not (
            p.extracted_text and len(p.extracted_text) >= (str_limit or float("inf"))
        )
    ]
    ...
    logger.info(
        f"Starting batch text extraction for {len(valid_pdfs_for_extraction)} PDFs."
    )
    ...
    for pdf_batch in batched(
        valid_pdfs_for_extraction, PDF_BATCH_SIZE
        ...
        try:
            ...
            for pdf_in_failed_batch in pdf_batch:
                ...
                pdfs_updated_in_db.append(
                    pdf_in_failed_batch
                )  # To save parsing_failed status
            continue

        for pdf_obj, result in zip(pdf_batch, extraction_results, strict=False):
            if result and result.content:
                ...
                pdf_obj.extracted_text_max_pages = (
                    max_pages  # Record intended max_pages
                )
                ...
                try:
                    ...
                    with open(md_file_path, "w", encoding="utf-8") as f_md:
                        f_md.write(content)
                    logger.debug(
                        f"Extracted text for PDF {pdf_obj.material_id} saved to {md_file_path.name}"
                    )
                except OSError as e_io:
                    logger.warning(
                        f"Could not write extracted text to .md file for {pdf_obj.material_id}: {e_io}"
                    )

            if result and result.metadata:
                metadata_dict = result.metadata
                title = str(metadata_dict.get("title", "") or "")
                if metadata_dict.get("subtitle"):
                    title += f" - {metadata_dict['subtitle']}"

                authors_list = metadata_dict.get("authors", [])
                author_str = (
                    ", ".join(authors_list)
                    if isinstance(authors_list, list)
                    else str(authors_list or "")
                )
                ...
                producer_val = metadata_dict.get("Producer") or metadata_dict.get(
                    "/Producer"
                )
        ...
        logger.info(
            f"{processed_count}/{len(valid_pdfs_for_extraction)} ({processed_count / len(valid_pdfs_for_extraction):.0%}) PDFs processed in batches."
        )
    ...
    if pdfs_updated_in_db:
        try:
            ...
            logger.info(
                f"Bulk updated {len(pdfs_updated_in_db)} PDF records in DB after batch extraction."
            )
        except Exception as e_db_update:
            logger.error(
                f"Error bulk updating PDFs in DB after batch extraction: {e_db_update}"
            )
...
async def extract_pdf_text(
    ...
    if (
        ...
        logger.warning(
            f"Invalid or non-existent PDF file path: {pdf_path}. Cannot parse."
        )
        return pdf
    ...
    current_max_len_setting = (
        pdf.extracted_text_max_length if pdf.extracted_text_max_length else 0
    )
    if (
        str_limit
        and current_text_len >= str_limit
        and current_max_len_setting >= str_limit
    ):
        logger.info(
            f"PDF {pdf_path.name} already has extracted text of sufficient length ({current_text_len}). Skipping."
        )
        return pdf
    ...
    try:
        logger.debug(
            f"Attempting direct text extraction for {pdf_path.name} (max_pages: {max_pages})."
        )
        ...
        extract_config = kreuzberg.ExtractionConfig(
            page_limit=max_pages if max_pages else 0
        )  # page_limit=0 means all pages for some backends
    ...
    if not extraction_result or not extraction_result.content:
        logger.info(
            f"Direct extraction yielded no content for {pdf_path.name}. Attempting OCR."
        )
        extraction_method_used = "ocr"
        try:
            ocr_config_paddle = kreuzberg.PaddleOCRConfig(
                language=OCR_LANGUAGE, use_gpu=OCR_USE_GPU
            )
            ocr_extract_config = kreuzberg.ExtractionConfig(
                force_ocr=True,
                ocr_backend="paddleocr",
                ocr_config=ocr_config_paddle,
                page_limit=max_pages if max_pages else 0,
            )
            extraction_result = await asyncio.wait_for(
                asyncio.to_thread(
                    extract_file, file_path=pdf_path, config=ocr_extract_config
                ),
    ...
    if not extraction_result or not extraction_result.content:
        logger.warning(
            f"All extraction attempts (direct and OCR) failed for {pdf_path.name}."
        )
        ...
        logger.info(
            f"Successfully extracted text (length {len(pdf_text_content)}, method: {extraction_method_used}) from {pdf_path.name}."
        )
        ...
        if extraction_result.metadata:
            ...
            pdf.title = (
                str(meta.get("title", pdf.title) or "") or None
            ...
            producer_val = meta.get("Producer") or meta.get(
                "/Producer"
            )  # Check both keys
        ...
        try:
            ...
            logger.debug(
                f"Extracted text for PDF {pdf.material_id} also saved to {md_file_path.name}"
            )
        except OSError as e_io_md:
            logger.warning(
                f"Could not write extracted text to .md file for {pdf.material_id}: {e_io_md}"
            )
    ...
    try:
        ...
        logger.error(
            f"Failed to save PDF model instance {pdf.material_id} after text extraction: {e_save}"
        )
    return pdf
...
def ocr_with_paddle(  # This function appears to be a standalone utility, not directly integrated.
    pdf_input_dir: Path,
    num_pages_to_ocr: int | None = PDF_TEXT_MAX_PAGES,  # Use configured default
    ...
    try:
        ...
        logger.error(
            f"Missing dependencies for ocr_with_paddle: {e_import}. Please install PyMuPDF, opencv-python, paddleocr, and Pillow."
        )
        return

    if not pdf_input_dir.exists() or not pdf_input_dir.is_dir():
        logger.error(
            f"PDF input directory {pdf_input_dir} does not exist or is not a directory."
        )
        return
    ...
    logger.info(
        f"Initializing PaddleOCR (lang='{OCR_LANGUAGE}', use_gpu={OCR_USE_GPU}). This may take a moment..."
    )
    try:
        ocr_engine = PaddleOCR(
            use_angle_cls=True,
            lang=OCR_LANGUAGE,
            page_num=num_pages_to_ocr,
            use_gpu=OCR_USE_GPU,
        )
    ...
    for pdf_path_obj in pdf_input_dir.glob("*.pdf"):
        ...
        if output_txt_path.exists():
            logger.info(
                f"Skipping {pdf_filename_stem}, output already exists: {output_txt_path.name}"
            )
            continue
        ...
        try:
            with fitz.open(pdf_path_obj) as pdf_doc:
                ...
                for page_num in range(pages_to_process):
                    ...
                    if (
                        ocr_result and ocr_result[0] is not None
                    ):  # Check if result is not None
                        for line_data in ocr_result[
                            0
                        ]:  # Result is a list of lists of lines
                            if (
                                line_data
                                and len(line_data) >= 2
                                and isinstance(line_data[1], tuple)
                            ):
            ...
            logger.info(
                f"Successfully OCR'd {pdf_filename_stem} to {output_txt_path.name}"
            )

        except Exception as e_ocr_process:
            logger.error(
                f"Error processing {pdf_filename_stem} with PaddleOCR: {e_ocr_process}"
            )


async def extract_metadata(pdf: PDF) -> PDF | None:
    """
    ...
    try:
        ...
        docinfo = (
            await asyncio.to_thread(lambda: dict(pdf_document.docinfo))
            if pdf_document.docinfo
            else {}
        )  # type: ignore
        ...
        logger.warning(
            f"PDF {pdf.path} is password protected. Marking as parsing_failed and deleting file."
        )
        pdf.parsing_failed = True
        await pdf.save(update_fields=["parsing_failed", "modified_at"])
        with contextlib.suppress(OSError):
            pdf.path.unlink(missing_ok=True)  # Delete the file
        ...
        logger.warning(
            f"Pikepdf error extracting metadata from {pdf.path}: {e_pikepdf}. Marking as parsing_failed."
        )
        pdf.parsing_failed = True
        await pdf.save(update_fields=["parsing_failed", "modified_at"])
        ...
        logger.error(
            f"Unexpected error extracting metadata from {pdf.path}: {e_generic}"
        )
    ...
    def _to_str_if_pdf_type(val: Any) -> str | None:
        if isinstance(val, (pikepdf.String, pikepdf.Name, pikepdf.Date)):
            return str(val)
        if val is None:
            return None
    ...
    if producer:
        update_dict["producer"] = producer

    # Date parsing for pikepdf dates (format D:YYYYMMDDHHMMSSOHH'mm')
    def parse_pdf_date(pdf_date_str: str | None) -> datetime | None:
        if not pdf_date_str:
            return None
            ...
            for fmt in formats_to_try:
                try:
                    return datetime.strptime(
                        pdf_date_str[: len(fmt)], fmt
                    )  # Match length of format
                except ValueError:
                    continue
    ...
    if creation_date_str:
        update_dict["file_creation_date"] = parse_pdf_date(creation_date_str)
    if mod_date_str:
        update_dict["file_modification_date"] = parse_pdf_date(mod_date_str)
...
async def enrich_pdfs(
    pdfs_queryset: QuerySet[PDF] | None = None,  # Allow passing a queryset
    input_mat_ids: list[int] | None = None,
    max_pages_text: int | None = PDF_TEXT_MAX_PAGES,
    char_limit_text: int | None = PDF_TEXT_STR_LIMIT,
) -> list[PDF]:
    """
    ...
    if pdfs_queryset is not None:
        ...
        logger.info(
            f"Received {len(pdfs_to_process)} PDFs from queryset for enrichment."
        )
    elif input_mat_ids:
        ...
        logger.info(
            f"Fetched {len(pdfs_to_process)} PDFs based on input material IDs for enrichment."
        )
    else:
        logger.info(
            "No specific PDFs or material_ids provided. Fetching all PDFs from DB for enrichment check."
        )
    ...
    pdfs_needing_metadata = [
        ...
        if not (
            ...
            or p.creator
            or p.producer
            or p.file_creation_date
            or p.file_modification_date
        )
    ]
    logger.info(
        f"Found {len(pdfs_needing_metadata)} PDFs potentially needing metadata extraction."
    )
    metadata_tasks = [extract_metadata(pdf) for pdf in pdfs_needing_metadata]
    await asyncio.gather(
        *metadata_tasks, return_exceptions=True
    )  # Results are PDF objects or None
    ...
    pdfs_needing_text = [
        p
        for p in pdfs_to_process  # Use the full list again, in case metadata step marked some as parsing_failed
        if (
            not p.extracted_text
            or (char_limit_text and len(p.extracted_text) < char_limit_text)
        )
        and not p.parsing_failed
    ]
    logger.info(
        f"Found {len(pdfs_needing_text)} PDFs potentially needing text extraction."
    )
    if pdfs_needing_text:
        # Using batch_extract_pdf_text (assuming it's more efficient for many files)
        await batch_extract_pdf_text(
            pdfs_needing_text, max_pages=max_pages_text, str_limit=char_limit_text
        )
...
async def deduplicate_pdfs(pdfs: list[PDF], compare_with_db: bool = True) -> list[PDF]:
    ...
    """
    try:
        from qdrant_client import QdrantClient  # Renamed models
        from qdrant_client import models as qdrant_models
    except ImportError:
        logger.warning(
            "qdrant_client not installed. Embedding-based deduplication will be skipped."
        )
        # Fallback to hash-based only if Qdrant is not available
        # For now, the embedding part is 'if False' anyway.

    logger.info(
        f"Starting deduplication process for {len(pdfs)} PDFs. Compare with DB: {compare_with_db}"
    )
    await init_tortoise_orm()
    ...
    if compare_with_db:
        ...
        logger.info(
            f"Found {len(already_replaced_ids)} PDFs already marked as duplicates in DB."
        )
    ...
    pdfs_for_hashing = sorted(
        [p for p in pdfs if p.material_id not in already_replaced_ids],
    ...
    if new_hash_mappings:
        ...
        for duplicate_id, original_id in new_hash_mappings.items():
            try:
                ...
                if (
                    pdf_to_update
                    and original_pdf_ref
                    and pdf_to_update.replace_with_id is None
                ):
                    pdf_to_update.replace_with = original_pdf_ref
                    await pdf_to_update.save(
                        update_fields=["replace_with_id", "modified_at"]
                    )
                    updated_count_by_hash += 1
                    already_replaced_ids.add(
                        duplicate_id
                    )  # Add to set to exclude from embedding part
            except Exception as e_save_hash:
                logger.error(
                    f"Error saving hash-based duplicate link for {duplicate_id} -> {original_id}: {e_save_hash}"
                )
...
def get_embedding(pdf: PDF) -> list[np.ndarray] | None:  # Type for list of numpy arrays
    """
    ...
    if not pdf.extracted_text:
        logger.debug(
            f"No extracted text for PDF {pdf.material_id}, cannot get embedding."
        )
        return None
    try:
        ...
        embedding_model = (
            TextEmbedding()
        )  # Initialize model (could be slow if done repeatedly)
