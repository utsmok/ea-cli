...
from fasthtml.components import (  # Specific imports
    H2,
    H4,
    H5,
    H6,
...
from monsterui.all import Label, LabelInput, UkIcon  # Specific imports from monsterui
...
from easy_access.settings import (  # For potential path/settings access
    SETTINGS,
    DirSetting,
)
...
def ItemDetailCard(
    ...
    *body_content: Any,  # Can be multiple FT objects or strings
    ...
    border_cls = (
        f"border-{color}-500"
        if color not in ["[var(--ring)]", "base"]
        else "border-[var(--ring)]"
    )
    header_bg_cls = (
        f"bg-{color}-100" if color not in ["[var(--ring)]", "base"] else base_bg_cls
    )

    card_classes = (
        f"{base_bg_cls} border {border_cls} rounded-lg shadow-sm {col_span_class}"
    )
    ...
    if lazy_load_url:
        ...
        summary_htmx_attrs = {
            ...
            "hx_swap": "innerHTML",
            "hx_trigger": "click toggle",  # "toggle" if summary click re-fetches
        }
    else:
    ...
    if tooltip:
        tooltip_icon = UkIcon(  # Assuming UkIcon handles tooltips or is styled for it
            "circle-help",
            cls="ml-1 text-xs cursor-help",  # Simpler classes
            height="12px",
            width="12px",
            title=tooltip,  # Standard title attribute for tooltip
        )

    content_div = Div(
        *actual_card_body_content, id=content_target_id, cls=content_classes
    )
    summary_element = Summary(
        H5(
            title,
            tooltip_icon,
            cls="font-semibold text-sm m-0 inline-flex items-center",
        ),  # Ensure items align
        chevron_icon,
        cls=summary_classes,
        **summary_htmx_attrs,
    ...
    return Details(
        summary_element, content_div, id=card_id, cls=card_classes, **details_attrs
    )
...
def create_checkbox_filter_group(
    filter_key: str,
    label_text: str,
    options: dict[str, str | LabelT],  # Value can be class string or LabelT enum
    current_values_str: str | None,  # Pipe-separated string of selected values
    current_total_items: int,
    option_counts: dict[str, int | str] | None = None,  # Counts for each option
) -> Div:
    """
    ...
    """
    logger.debug(
        f"Creating checkbox group for: {filter_key}, current values: '{current_values_str}', counts: {option_counts}"
    )
    selected_values_set: Set[str] = (
        set(current_values_str.split("|")) if current_values_str else set()
    )
    ...
    def get_sort_key(option_tuple: tuple[str, str | LabelT]) -> tuple[int, int, str]:
        val, _ = option_tuple
    ...
    for option_value_str, style_or_class in sorted_options_list:
        safe_html_id_suffix = "".join(
            c if c.isalnum() else "-" for c in option_value_str
        )  # For HTML IDs
        current_option_count = (
            option_counts.get(option_value_str, "") if option_counts else ""
        )
        ...
        is_disabled_bool = (
            isinstance(current_option_count, int)
            and current_option_count == 0
            and not is_checked_bool
        ) or (
            current_option_count == "-" and not is_checked_bool
        )  # Explicitly no items
        ...
        if filter_key != "faculty":  # Faculty uses predefined badge-color strings
            if isinstance(
                style_or_class, LabelT
            ):  # Map LabelT to specific badge classes
                if style_or_class == LabelT.primary:
                    checkbox_color_class, badge_style_class = (
                        "checkbox-accent",
                        "badge badge-neutral",
                    )
                elif style_or_class == LabelT.secondary:
                    checkbox_color_class, badge_style_class = (
                        "checkbox-primary",
                        "badge badge-primary",
                    )
                elif style_or_class == LabelT.destructive:
                    checkbox_color_class, badge_style_class = (
                        "checkbox-error",
                        "badge badge-error",
                    )
        ...
        if is_checked_bool:  # If currently selected
            count_display_str = (
                f"= {current_total_items}"  # Show total items for active filter
            )
            count_tooltip_str = (
                "Currently selected. Shows total items matching all active filters."
            )
        elif isinstance(current_option_count, int):
            count_display_str = (
                f"▼ {current_option_count}"
                if current_option_count < current_total_items
                else f"▲ {current_option_count}"
            )
            if (
                current_option_count == current_total_items and current_active_filters
            ):  # current_active_filters from closure
                count_display_str = f"= {current_option_count}"  # No change if added
            count_tooltip_str = (
                f"Adds/removes this option. Resulting count: {current_option_count}."
            )
        else:  # count is "-" or other string
            count_display_str = (
                ""  # Don't show count for globally empty options unless selected
            )
        ...
        if is_disabled_bool:
            disabled_options_display_list.append(
                Span(
                    option_value_str
                    if len(option_value_str) < 18
                    else option_value_str[:16] + "…",
                    cls="text-xs opacity-40 mr-2 cursor-not-allowed",
                    title=f"{option_value_str} (no items)",
                )
            )
        else:
            count_badge_span = (
                Span(
                    count_display_str,
                    title=count_tooltip_str,
                    cls=f"{badge_style_class} badge-outline ml-1 align-middle text-xs cursor-help",
                )
                if count_display_str
                else Span()
            )

            htmx_vals_for_cb = {
                form_field_name: option_value_str,
                "checked": str(not is_checked_bool).lower(),
            }

            checkbox = fh.CheckboxX(  # Assuming CheckboxX is a custom or fasthtml component
                name=form_field_name,
                value=option_value_str,
                id=f"cb-{filter_key}-{safe_html_id_suffix}",
                checked=is_checked_bool,
                cls=f"checkbox checkbox-xs align-middle mr-1 {checkbox_color_class}",
                hx_post=str(urls.URLS[Url.data_grid]),
                hx_target="#data-grid-component",
                hx_swap="outerHTML",
                hx_trigger="change",
                hx_vals=json.dumps(htmx_vals_for_cb),
                disabled=is_disabled_bool,
            )
            badge_text_label = Span(
                option_value_str, cls=f"{badge_style_class} mr-1 cursor-pointer text-xs"
            )

            wrapper_label_for_cb = fh.Label(
                checkbox,
                badge_text_label,
                count_badge_span,
                cls="flex items-center gap-x-1 py-0 px-1 mb-0.5 rounded min-h-0",
                html_for=f"cb-{filter_key}-{safe_html_id_suffix}",
            )  # Use html_for
        ...
        group_card_content.append(
            Div(
                UkIcon(
                    ...
                    title="Options with no items currently",
                ),
                *disabled_options_display_list,
                cls="flex flex-wrap items-center gap-x-1 gap-y-0.5 mt-1 mb-0.5",
            )
        )
...
def render_course_details(item_data: dict[str, Any]) -> tuple[FT, ...]:
    ...
    if not courses_data:
        return (
            P(
                "No course data associated with this item.",
    ...
    for course in courses_data:
        ...
        for person in persons_in_course:
            ...
            for org in orgs_of_person:
                if org.get(
                    "full_abbreviation"
                ):  # Prefer full_abbreviation if available
                    all_course_org_abbreviations.add(str(org["full_abbreviation"]))

    if all_course_org_abbreviations:
        course_items_list.append(
            fh.Div(cls="my-2 border-t border-base-300")
        )  # Using fh.Div for Divider
        course_items_list.append(
            fh.Strong(
                "Related Organizations (from Course Contacts):",
                cls="text-xs font-medium block mb-1",
            )
        )
        course_items_list.append(
            Div(cls="flex flex-wrap gap-1")(
                *[
                    Label(org_abbr, cls=f"badge badge-sm {DEFAULT_PILL_STYLE}")
                    for org_abbr in sorted(list(all_course_org_abbreviations))
                ]
            )
        )
...
def render_contact_info(item_data: dict[str, Any]) -> tuple[FT, ...]:
    ...
    if all_contact_emails_set:
        ...
        copy_js_onclick = (
            f"navigator.clipboard.writeText('{emails_str}'); event.target.textContent='Copied!';"
    ...
    if not contact_persons_list:
        rendered_items.append(
            P(
                "No contact persons found for this item's courses.",
                cls="text-sm text-base-content/70",
            )
        )
    else:
        ...
        for person in sorted(
            contact_persons_list, key=lambda p: p.get("main_name", "zzzz")
        ):  # Sort by name
            person_id = person.get("id")
            if person_id in unique_person_ids_processed:
                continue
            ...
            if orgs:  # Find highest level orgs (smallest hierarchy_level)
                min_level = min(
                    (
                        o.get("hierarchy_level", float("inf"))
                        for o in orgs
                        if o.get("hierarchy_level") is not None
                    ),
                    default=float("inf"),
                )
                if min_level != float("inf"):
                    top_org_abbrs_set = {
                        str(o["full_abbreviation"])
                        for o in orgs
                        if o.get("hierarchy_level") == min_level
                        and o.get("full_abbreviation")
                    }

            rendered_items.append(
                Div(
                    cls="flex items-center justify-between space-x-2 py-1.5 border-b border-base-200 last:border-b-0"
                )(
                    Span(
                        str(person.get("main_name", "Unknown Name")),
                        cls="text-sm truncate",
                        title=str(person.get("main_name", "Unknown Name")),
                    ),
                    Div(cls="flex items-center space-x-1 flex-shrink-0")(
                        *[
                            Label(abbr, cls=f"badge badge-xs {DEFAULT_PILL_STYLE}")
                            for abbr in sorted(list(top_org_abbrs_set))
                        ],  # Badges for orgs
                        A(
                            UkIcon("mail", cls="w-4 h-4"),
                            href=f"mailto:{email_addr}",
                            title=f"Email {email_addr}",
                            cls="link text-primary",
                        )
                        if email_addr
                        else Span(
                            UkIcon("mail-off", cls="w-4 h-4 text-base-content/50"),
                            title="No email",
                        ),  # mail-off icon
                        A(
                            UkIcon("external-link", cls="w-4 h-4"),
                            href=str(people_page),
                            target="_blank",
...
def render_teacher_info(item_data: dict[str, Any]) -> tuple[FT, ...]:
    ...
    if not all_persons_map:
        return (
            P(
                "No teachers or other persons found for this item's courses.",
                cls="text-sm text-base-content/70",
            ),
        )

    sorted_persons_list = sorted(
        all_persons_map.values(), key=lambda p: str(p.get("main_name", "zzzz"))
    )
...
def render_item_history(history_records: list[ItemUpdate]) -> FT:  # Changed input type
    ...
    for record in history_records:
        ...
        modified_by_str = str(
            change_details_dict.get("modified_by", "Unknown User")
        ...
        if isinstance(change_details_dict, dict):
            for field_key, change_val_dict in change_details_dict.items():
                if field_key in [
                    ...
                    continue
                if isinstance(
                    change_val_dict, dict
                ):  # Expected structure: {"old": "val1", "new": "val2"}
        ...
        if not formatted_changes_list:
            formatted_changes_list.append(
                "No specific field changes logged (or only meta fields updated)."
            )
        ...
        """
        timeline_log_entries.append(
            Li(NotStr(log_entry_html))
        )  # NotStr tells fasthtml it's raw HTML

    return ItemDetailCard(  # Use the standard card component
        title="Change History",
        body_content=(
            Ul(*timeline_log_entries, cls="space-y-3 list-none p-0 m-0"),
        ),  # Pass content as tuple
...
def render_table_rows(
    ...
    if (
        df_slice is not None and not df_slice.is_empty()
    ):  # df_slice.height > 0 changed to not df_slice.is_empty()
        ...
        for row_data_dict in df_slice.select(cols_to_render).iter_rows(named=True):
            ...
            if material_id_val is None:
                continue  # Skip row if no material_id
            ...
            for col_name in cols_to_render:
                ...
                if col_name == "url" and cell_value:
                    cell_display_content = A(
                        ...
                        title=f"Open link: {cell_value}",
                        cls="link link-hover text-primary hover:text-primary-focus",
                        onclick="event.stopPropagation();",
                    )
                elif (
                    col_name in BADGE_STYLES
                    or "classification" in col_name
                    or "prediction" in col_name
                ):
                    style_key_map = (
                        BADGE_STYLES.get("classification")
                        if "classification" in col_name or "prediction" in col_name
                        else BADGE_STYLES.get(col_name, {})
                    )
                    style_class_enum = style_key_map.get(
                        str(cell_value).lower() if cell_value else "none",
                        DEFAULT_PILL_STYLE,
                    )  # Use lower for matching
                    style_class_str = str(
                        style_class_enum
                    )  # Convert LabelT to string class

                    display_text_for_badge = (
                        cell_display_text.split("-")[0].strip()
                        if "-" in cell_display_text
                        else cell_display_text.strip()
                    )
                    cell_display_content = Label(
                        display_text_for_badge,
                        title=cell_display_text,
                        cls=f"{style_class_str} badge-sm cursor-help",
                        onclick="event.stopPropagation();",
                    )
                else:  # Default text rendering with truncation
                    cell_display_content = (
                        display_text_for_badge[:MAX_CELL_DISPLAY_LENGTH] + "..."
                        if len(display_text_for_badge) > MAX_CELL_DISPLAY_LENGTH
                        else display_text_for_badge
                    )
                ...
                table_cells.append(
                    Td(
                        cell_display_content,
                        title=cell_display_text,
                        cls=f"py-2 px-3 border-b border-base-200/80 {td_cls_name}",
                    )  # Adjusted padding
                )

            # Construct modal URL using the Url enum and replacing placeholder
            modal_url_str = str(urls.URLS[Url.show_item_details]).replace(
                "{material_id:int}", str(material_id_val)
            )
...
def page_header_component(
    user_details: dict[str, str | None],  # User details from session/auth
    app_state: AppState,  # Current application state
    filter_counts: dict[str, dict[str, int | str]],  # Pre-calculated filter counts
    ...
    if user_faculty_display == "all":
        user_faculty_display = "All items"
    ...
    filter_checkbox_groups_list: list[FT] = [
        create_checkbox_filter_group(
            ...
            current_values_str=current_active_filters.get("workflow_status"),
            option_counts=filter_counts.get("workflow_status"),
        ...
        create_checkbox_filter_group(
            ...
            current_values_str=current_active_filters.get("status"),
            option_counts=filter_counts.get("status"),
            current_total_items=total_filtered_rows,
        ),
        create_checkbox_filter_group(  # Using "classification" style for both actual and manual classification
            ...
            label_text="System Classification",
            options=BADGE_STYLES["classification"],
            current_values_str=current_active_filters.get("classification"),
            option_counts=filter_counts.get("classification"),
            current_total_items=total_filtered_rows,
        ),
        create_checkbox_filter_group(
            ...
            options=BADGE_STYLES["classification"],
            current_values_str=current_active_filters.get("manual_classification"),
            option_counts=filter_counts.get("manual_classification"),
            current_total_items=total_filtered_rows,
        ),
    ]
    if (
        is_admin_user
        or not user_details.get("faculty")
        or user_details.get("faculty") == "all"
    ):
        filter_checkbox_groups_list.append(
            create_checkbox_filter_group(
                filter_key="faculty",
                label_text="Faculty",
                options=BADGE_STYLES["faculty"],
                current_values_str=current_active_filters.get("faculty"),
                option_counts=filter_counts.get("faculty"),
                current_total_items=total_filtered_rows,
            )
        )
    ...
    for col_name in FILTERABLE_COLUMNS:
        if (
            col_name not in cols_for_checkboxes
        ):  # Only add if not already a checkbox group
            ...
            text_filter_inputs_list.append(
                LabelInput(  # Assuming LabelInput is a monsterui component
                    label=field_label,
                    name=form_input_name,
                    value=current_active_filters.get(col_name, ""),
                    placeholder="Filter...",
                    id=input_id_str,
    ...
    filter_form_content = Form(
        Div(  # Button container at top of sidebar
            fh.Button(
                UkIcon("rotate-ccw", cls="mr-1 w-4 h-4"),
                "Reset All Filters",
                type="button",
                hx_post=str(urls.URLS[Url.data_grid]),
                hx_target="#data-grid-component",
        ...
        *text_filter_inputs_list,
        id="filter-form",
        cls="flex flex-col gap-y-1 p-2",  # Added padding to form
    )

    # --- Sidebar Structure (using Alpine.js for toggle) ---
    sidebar_component = Div(
        x_data="{ slideOverOpen: false }", cls="relative z-50"
    )(  # Higher z-index
        ...
        Template(x_teleport="body")(
            Div(
                ...
                **{"@keydown.window.escape": "slideOverOpen=false"},
                cls="fixed inset-0 overflow-hidden",
                x_cloak=True,
                hx_ext="preload",
            )(  # hx_ext for preloading if needed
                Div(
                    x_show="slideOverOpen",
                    x_transition_opacity_duration_600ms=True,  # fasthtml specific transition
                    **{"@click": "slideOverOpen = false"},
                    cls="absolute inset-0 bg-black bg-opacity-25 backdrop-blur-sm",
                ),
                Div(
                    cls="fixed inset-y-0 left-0 flex max-w-full pr-10"
                )(  # Sidebar container
                    Div(
                        x_show="slideOverOpen",
                        **{"@click.away": "slideOverOpen = false"},
                        ...
                        cls="w-screen max-w-xs md:max-w-sm",
                    )(  # Responsive width
                        Div(
                            cls="flex flex-col h-full overflow-y-auto bg-base-200 border-r border-base-300 shadow-xl"
                        )(
    ...
    if raw_data_dir and raw_data_dir.exists:
        ...
        try:
            latest_file_mod_time = max(
                (f.modified for f in raw_data_dir.files if f.exists), default=None
            )
            if latest_file_mod_time:
                last_update_display = latest_file_mod_time.strftime("%Y-%m-%d %H:%M")
        except Exception as e_stat:
            logger.debug(
                f"Could not determine last update time from file system: {e_stat}"
            )

    return Div(  # Main header container
        Div(
            cls="flex flex-wrap justify-between items-center mb-4 gap-y-2"
        )(  # Responsive wrap
            Div(  # Title and last update
                H4(
                    "Copyright Data Dashboard",
                    cls="text-xl md:text-2xl font-bold text-primary",
                ),
                P(
                    f"Data last updated: {last_update_display}",
                    cls="text-xs text-base-content/70 mt-1",
                ),
            ),
            Div(
                cls="flex items-center space-x-3 md:space-x-4"
            )(  # User info and actions
                sidebar_component,  # Filter button and sidebar
                Div(cls="flex items-center space-x-2")(  # Avatar and user name
                    Img(
                        src=f"https://api.dicebear.com/9.x/initials/svg?seed={user_details.get('name', 'User')}&backgroundColor=00897b,00acc1,039be5,3949ab,5e35b1,8e24aa&backgroundType=gradientLinear&radius=50",
                        alt="avatar",
                        cls="w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-base-300 shadow-sm",
                    ),
                    Div(
                        Span(
                            f"Hi, {user_details.get('name', 'User')}!",
                            cls="text-sm font-medium hidden md:block",
                        ),
                        Span(
                            f"Faculty: {user_faculty_display}",
...
def render_data_grid_component(
    ...
    for col_name_to_display in DISPLAY_COLUMNS:
        ...
        header_display_text = (
            "ID"
            if col_name_to_display == "material_id"
            else col_name_to_display.replace("_", " ").title()
        )

        sort_icon_class = ""
        next_sort_direction_is_desc = (
            False  # Default to ASC for next click if not current sort col
        )

        if col_name_to_display == app_state.sort_by:  # This is the current sort column
            next_sort_direction_is_desc = (
                not app_state.sort_desc
            )  # Toggle current direction
            sort_icon_class = "arrow-down" if app_state.sort_desc else "arrow-up"

        sort_htmx_vals = json.dumps(
            {
                "sort_by": col_name_to_display,
                "sort_desc": str(next_sort_direction_is_desc).lower(),
            }
        )

        header_button_content: list[Any] = [header_display_text]
        if sort_icon_class:
            header_button_content.append(
                UkIcon(sort_icon_class, cls="inline-block ml-1.5 w-3 h-3")
            )
            ...
            Th(
                Button(
                    *header_button_content,
                    hx_get=str(urls.URLS[Url.data_grid]),
                    hx_target="#data-grid-component",
                    hx_vals=sort_htmx_vals,
    ...
    table_body = Tbody(
        *render_table_rows(df_slice), id="data-table-body"
    )  # Pass app_state if render_table_rows needs it
    ...
    table_component = Div(
        Table(
            ...
            role="grid",
            id="data-table",
            cls="table table-sm table-zebra w-full",
        ),  # Added table-zebra
        id="table-wrapper",
    ...
    pagination_component = render_pagination(
        app_state, total_pages, total_filtered_rows
    )
    ...
    return Div(
        ...
        Div(
            table_component,
            cls="htmx-indicator-opacity opacity-100 transition-opacity duration-300",
        ),  # Simpler opacity indicator
        id="data-grid-component",
        cls="flex flex-col",  # Removed justify-center
    )
...
def render_pagination(
    ...
    """
    if (
        total_pages <= 1 and total_items <= app_state.per_page
    ):  # Hide if only one page and not many items
    ...
    end_item_idx: int = min(
        app_state.per_page * current_page, total_items
    )  # Ensure it doesn't exceed total items
    ...
    pages_to_show_set: Set[int | str] = {1, total_pages, current_page}
    if current_page > 1:
        pages_to_show_set.add(current_page - 1)
    ...
    if total_pages > 3:
        pages_to_show_set.add(2)
    if total_pages > 4:
        pages_to_show_set.add(total_pages - 1)

    sorted_page_numbers = sorted(
        [
            p
            for p in list(pages_to_show_set)
            if isinstance(p, int) and 1 <= p <= total_pages
        ]
    )

    # Add ellipses "..." where there are gaps
    final_page_display_elements: list[int | str] = []
    last_num_added = 0
    for num in sorted_page_numbers:
        if (
            num > last_num_added + 1 and last_num_added != 0
        ):  # Check if there's a gap after the first element
    ...
    page_buttons.append(
        A(
            ...
            role="button",
            cls=btn_disabled_cls if current_page <= 1 else btn_enabled_cls,
        )
    )
    page_buttons.append(
        A(
        ...
        if page_num_display == "...":
            ...
            page_buttons.append(
                A(
                    ...
                    hx_get=str(urls.URLS[Url.data_grid]),
                    hx_target="#data-grid-component",
                    hx_vals=json.dumps({"page": page_num_int}),
                    role="button",
                    cls=num_selected_cls
                    if page_num_int == current_page
                    else num_other_cls,
                )
            )
    ...
    page_buttons.append(
        A(
            ...
            hx_get=str(urls.URLS[Url.data_grid]),
            hx_target="#data-grid-component",
            hx_vals=json.dumps({"page": min(total_pages, current_page + 1)}),
            role="button",
            cls=btn_disabled_cls if current_page >= total_pages else btn_enabled_cls,
        )
    )
    page_buttons.append(
        A(
            ...
            hx_get=str(urls.URLS[Url.data_grid]),
            hx_target="#data-grid-component",
            hx_vals=json.dumps({"page": total_pages}),
            role="button",
            cls=btn_disabled_cls if current_page >= total_pages else btn_enabled_cls,
        )
    )
    ...
    info_text = P(
        f"Showing {start_item_idx} to {end_item_idx} of {total_items} results.",
        cls="text-sm text-base-content/80",
    )
...
def render_modal_field(col_name: str, value: Any) -> tuple[FT, str]:
    ...
    if col_name == "url" and value:
        return A(
            UkIcon("external-link", cls="w-4 h-4 inline-block mr-1"),
            "Open Link",  # Clearer text
            href=str(value),
            target="_blank",
            rel="noopener noreferrer",
            title=f"Open link: {value}",
            cls="link link-hover text-primary hover:text-primary-focus text-sm",
        ), "a"
    elif col_name == "url" and not value:
        return Span(
            UkIcon("unlink", cls="w-4 h-4 inline-block text-base-content/50 mr-1"),
            "No URL",
    ...
    if (
        "classification" in col_name or "prediction" in col_name
    ):  # Use "classification" styles for these
        style_lookup_key = "classification"

    if style_lookup_key in BADGE_STYLES:
        style_info = BADGE_STYLES[style_lookup_key].get(
            val_str, DEFAULT_PILL_STYLE
        )  # Use full val_str for lookup
        badge_class = (
            str(style_info) if isinstance(style_info, LabelT) else style_info
        )  # style_info could be a class string
        return Label(
            display_text, cls=f"{badge_class} badge-sm"
        ), "label"  # Using monsterui Label
...
def render_labelled_item(
    label_text: str, content_component: FT, content_tag_type: str = "span"
) -> Div:
    """
    ...
    """
    label_element = fh.Strong(
        label_text,
        cls="block text-xs font-semibold text-base-content/90 mb-0.5 tracking-wide uppercase",
    )
    ...
    if (
        content_tag_type == "label"
    ):  # Indicates it's a badge/pill from render_modal_field
    ...
    return Div(
        label_element, content_element, cls="mb-2"
    )  # Overall container for one labelled item
...
def create_editable_pill_div(
    ...
    options_map: dict[str, str | LabelT],  # {option_value: style_enum_or_class, ...}
    material_id: int,
) -> Div:
    """
    ...
    if hasattr(display_component, "attrs") and isinstance(
        display_component.attrs, dict
    ):
        ...
        dropdown_list_items.append(
            Li(
                A(
                    Label(
                        str(option_key),
                        cls=f"{style_class_str} badge-sm w-full text-left",
                    ),  # Full width label
                    href="#",
                    onclick=js_onclick_str,
                    hx_post=str(urls.URLS[Url.update_single_field]),
                    hx_vals=htmx_values_dict,
                    hx_swap="none",
    ...
    dropdown_container = Div(
        Ul(
            *dropdown_list_items, cls="uk-nav uk-dropdown-nav p-1"
    ...
    pill_trigger_area = Div(
        display_component, cls="inline-block uk-inline cursor-pointer"
    )
    ...
    return Div(
        ...
        Div(
            pill_trigger_area, dropdown_container, cls="text-right"
        ),  # Align pill to the right
        cls="flex items-center justify-between space-x-2 mb-2",
    )


def create_readonly_item_div(
    current_value: int | str | float | bool | datetime.date | datetime.datetime | None,
    label_text: str,
    ...
    return render_labelled_item(
        label_text, content_component, content_tag_type="readonly"
    )
