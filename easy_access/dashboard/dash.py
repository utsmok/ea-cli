...
from easy_access.dashboard.data import (
    ProcessedDataResult,
    fetch_data,  # ProcessedDataResult for type hint
...
from easy_access.dashboard.web import (  # Renamed for clarity
    LOGIN_REDIRECT_RESPONSE,
    Login,
    load_app_state,
    users,
)
from easy_access.dashboard.web import auth_bware as dashboard_auth_bware
from easy_access.db.retrieve import (
    retrieve_osiris_data,  # For specific Osiris data route
)
...
if dashboard_static_path.exists() and dashboard_static_path.is_dir():
    app.mount(
        "/static",
        StaticFiles(directory=dashboard_static_path),
        name="static_dashboard_assets",
    )
else:
    logger.warning(
        f"Dashboard static directory not found at {dashboard_static_path}. Static files may not load."
    )
...
@rt(
    dashboard_urls_module.URLS[Url.save_item_details], methods=["POST"]
)  # Use URLS from module
async def save_item_details_route(
    ...
    """
    logger.info(
        f"Saving details for material_id: {material_id}. New remarks length: {len(remarks)}"
    )
    ...
    try:
        await store_item_changes(
            [update_data_dict], auth_details
        )  # store_item_changes expects a list
        add_toast(
            session, f"Details for item {material_id} saved successfully.", "success"
        )
        # Trigger client-side event if needed, e.g., to update parts of the modal
        return StarletteResponse(
            status_code=200,
            headers=HtmxResponseHeaders(trigger="remarksSaveSuccess").headers,
        )
    except Exception as e:
        logger.error(
            f"Error saving details for material_id {material_id}: {e}", exc_info=True
        )
        add_toast(
            session,
            f"Error saving details for item {material_id}: {str(e)[:100]}",
            "error",
        )
...
async def update_single_field_route(
    ...
    """
    logger.info(
        f"Updating single field for material_id {material_id}: {field_name} = '{value}'"
    )
    ...
    try:
        ...
        logger.error(
            f"Error updating single field '{field_name}' for material_id {material_id}: {e}",
            exc_info=True,
        )
...
@rt(
    dashboard_urls_module.URLS[Url.get_osiris_data]
)  # Path includes placeholder {material_id:int}
async def get_osiris_data_route(
    material_id: int,
) -> list[dict[str, Any]]:  # Type hint for return
    """
    ...
    try:
        ...
        data: list[dict[str, Any]] = await asyncio.to_thread(
            retrieve_osiris_data, [material_id]
        )
        ...
        logger.error(
            f"Error in get_osiris_data route for material_id {material_id}: {e}",
            exc_info=True,
        )
...
async def get_pdf_element_route(material_id: int) -> Embed | Div:  # FT is too general
    ...
    if not pdf_dir or not pdf_dir.exists:
        logger.error(
            "PDF download directory not configured or found. Cannot serve PDF element."
        )
    ...
    if not pdf_file_path.exists():
        ...
        if alt_files:
            ...
            logger.warning(
                f"PDF file for material_id {material_id} not found at expected paths."
            )
            return Div(
                f"PDF file for item {material_id} not found.", cls="text-error p-4"
            )
    ...
    return Embed(
        src=pdf_serve_url, type="application/pdf", width="100%", height="800px"
    )
...
async def get_file_route(material_id: int) -> FileResponse | HTMLResponse:
    ...
    if not pdf_dir or not pdf_dir.exists:
        ...
        return HTMLResponse(
            "Server configuration error: PDF directory not set.", status_code=500
        )
    ...
    if not file_path.exists():
        ...
        if alt_files:
            ...
            logger.warning(
                f"File not found for material_id {material_id} at {file_path} (and alternatives)."
            )
            return HTMLResponse(
                f"File for item {material_id} not found.", status_code=404
            )

    return FileResponse(
        file_path, media_type="application/pdf", filename=file_path.name
    )


@rt(dashboard_urls_module.URLS[Url.get_extracted_text_element], methods=["GET"])
async def get_extracted_text_element_route(
    material_id: int,
) -> Div:  # FT changed to Div
    """
    ...
    text_content: str = get_extracted_text(
        material_id
    )  # This is a synchronous function call
    # Wrap the text content, allowing HTML rendering via NotStr
    return Div(
        NotStr(text_content), cls="p-2 prose max-w-none"
    )  # prose for tailwind typography if used


@rt(
    dashboard_urls_module.URLS[Url.get_entities_element]
)  # Path includes {material_id:int}
async def get_entities_element_route(material_id: int) -> Div:  # FT changed to Div
    ...
    entities_obj: Entities | None = get_entities(material_id)  # Synchronous call

    if not entities_obj or not entities_obj.items:
        return Div(
            P("No entities found or extracted for this item."), cls="p-4 text-sm"
        )

    # Get grouped and sorted entities
    grouped_entities_dict, sorted_labels_list = (
        entities_obj.get_grouped_and_sorted_entities()
    )
    ...
    for label in sorted_labels_list:
        ...
        for entity in entities_in_group:
            text_counts[entity.text] += 1

        processed_texts_in_group: Set[str] = set()
        for entity in sorted(
            entities_in_group, key=lambda e: e.text.lower()
        ):  # Sort entities alphabetically by text
            if entity.text in processed_texts_in_group:
                continue  # Show each unique text once
            processed_texts_in_group.add(entity.text)

            count_badge_str = (
                f" ({text_counts[entity.text]}x)"
                if text_counts[entity.text] > 1
                else ""
            )
            ...
            if entity.score is not None:
                ...
                if entity.score < 0.7:
                    score_color_cls = "badge-warning"  # Low confidence
                elif entity.score >= 0.9:
                    score_color_cls = "badge-success"  # High confidence
                else:
                    score_color_cls = "badge-info"  # Medium confidence

            entity_list_items.append(
                Li(
                    Span(entity.text + count_badge_str, cls="text-sm"),
                    Span(score_display, cls=f"ml-2 badge badge-xs {score_color_cls}")
                    if score_display
                    else Span(),
        ...
        entity_display_sections.append(
            Div(
                H5(
                    ...
                    cls="font-semibold text-xs uppercase tracking-wider mb-1 text-primary",
                ),
...
async def main_data_grid_route(
    ...
    if request.method == "POST":  # Check if form data is expected
        try:
            ...
            logger.warning(
                f"Could not parse form data for request to {request.url.path}: {e_form}"
            )

    # Combine query parameters and form data (form data takes precedence for same keys)
    request_params_combined: dict[str, Any] = {
        **dict(request.query_params),
        **form_data,
    }
    logger.info(
        f"Request to data_grid: Method={request.method}, Params={request_params_combined}"
    )
    ...
    processed_data_result: ProcessedDataResult = await fetch_data(
        current_app_state, session
    )
    ...
    if processed_data_result.app_state.page != current_app_state.page:
        logger.info(
            f"Page validated: original {current_app_state.page}, final {processed_data_result.app_state.page}. Updating session."
        )
    ...
    user_faculty_for_oob: str | None = auth_details_for_oob.get("faculty")
    ...
    if (
        is_admin_user_for_oob
        or not user_faculty_for_oob
        or user_faculty_for_oob == "all"
    ):
    ...
    for key, label, opts_map in checkbox_groups_config:
        ...
        if hasattr(
            group_component, "attrs"
        ):  # Ensure it's a component fasthtml can add attrs to
    ...
    if is_htmx_request:
        logger.debug(
            "HTMX request detected. Returning data grid and OOB filter updates."
        )
        ...
        modal_dialog_placeholder = Dialog(
...
@rt(
    dashboard_urls_module.URLS[Url.show_item_details]
)  # Path includes {material_id:int}
async def show_item_details_modal_route(
    session: dict[str, Any], material_id: int
) -> tuple[FT, ...]:
    """
    ...
    auth_details: dict[str, Any] = session.get("auth", {})
    faculty_constraint_for_nav: dict[str, str] | None = None
    user_faculty_for_nav: str | None = auth_details.get("faculty")
    if (
        auth_details.get("role") != "admin"
        and user_faculty_for_nav
        and user_faculty_for_nav != "all"
    ):
    ...
    try:
        ...
        ordered_df_for_nav = get_filtered_sorted_df(
            current_app_state, extra_constraints=faculty_constraint_for_nav
        )
        if "material_id" in ordered_df_for_nav.columns:
            ordered_ids_list: list[Any] = ordered_df_for_nav.get_column(
                "material_id"
            ).to_list()
            ...
            try:
                ...
                current_idx = (
                    str_ordered_ids.index(str_material_id)
                    if str_material_id in str_ordered_ids
                    else -1
                )

                if current_idx != -1:
                    if current_idx > 0:
                        prev_item_id = int(ordered_ids_list[current_idx - 1])
                    if current_idx < len(ordered_ids_list) - 1:
                        next_item_id = int(ordered_ids_list[current_idx + 1])
            except ValueError:  # material_id not found in list or type issue
                logger.warning(
                    f"Material ID {material_id} not found in the current filtered/sorted list for modal navigation."
                )
        else:
            logger.warning(
                "'material_id' column not found in DataFrame for modal navigation."
            )

    except Exception as e_nav:
        logger.error(
            f"Error determining prev/next navigation for modal (material_id {material_id}): {e_nav}"
        )
    ...
    item_details_list: list[dict[str, Any]] = await asyncio.to_thread(
        retrieve_osiris_data, [material_id]
    )

    if not item_details_list:
        logger.warning(
            f"No detailed data found for material_id {material_id} via retrieve_osiris_data."
        )
        ...
        error_modal_backdrop = Form(method="dialog", cls="modal-backdrop")(
            NotStr("<button>close</button>")
        )
        return (error_modal_box, error_modal_backdrop), HtmxResponseHeaders(
    ...
    filename_display: FT = H4(
        filename_str, cls="font-semibold text-lg break-all leading-tight"
    )
    if file_url_str:
        filename_display = A(
            filename_display,
            href=file_url_str,
            target="_blank",
            cls="link hover:link-primary",
        )

    status_pill_component, _ = render_modal_field(
        "status", item_data_dict.get("status")
    )
    material_id_pill = Label(
        f"ID: {material_id}", cls=f"{DEFAULT_PILL_STYLE} badge-sm ml-2"
    )

    modal_header = Div(
        cls="flex items-start justify-between space-x-4 pb-2 border-b border-base-300"
    )(
        Div(cls="flex items-center space-x-3 flex-grow min-w-0")(
            status_pill_component, material_id_pill, filename_display
        ),
        Form(method="dialog")(
    ...
    data_entry_card_content = (
        create_editable_pill_div(
            item_data_dict.get("workflow_status"),
            "Workflow Status",
            "workflow_status",
        ...
        create_editable_pill_div(
            item_data_dict.get("manual_classification"),
            "Manual Classification",
            "manual_classification",
            BADGE_STYLES["classification"],
            material_id,
        ...
        Div(
            fh.Strong(
                "Remarks",
                cls="block text-xs font-semibold uppercase tracking-wider text-base-content/70 mb-1",
            ),
            TextArea(
                name="remarks",
                rows="4",
        ...
        Div(cls="flex justify-end space-x-2 mt-3")(  # Buttons for remarks
            Button(
                ...
                **{
                    "@click": "remarks = originalRemarks; document.getElementById('modal_save_remarks_btn').disabled = true;"
                },
            ),
            Button(
                "Save Remarks",
                id="modal_save_remarks_btn",
                type="submit",
    ...
    item_info_card_content = tuple(
        create_readonly_item_div(
            item_data_dict.get(key), key.replace("_", " ").title(), key
        )
        for key in [
            "classification",
            "ml_prediction",
    ...
    text_details_card_content = tuple(
        ...
        for key in ["title", "author", "publisher", "doi", "isbn"]
        if key in item_data_dict
    )
    # Counts (Page, Word, Picture)
    counts_card_content = tuple(
        create_readonly_item_div(
            item_data_dict.get(key), key.replace("count", "").title(), key
        )
        for key in ["pagecount", "wordcount", "picturecount"]
        if key in item_data_dict
    )
    # Osiris Data Cards
    ...
    item_history_display_card = render_item_history(
        await get_item_history_for_dashboard(material_id)
    )
    ...
    modal_cards = Div(cls="grid grid-cols-1 md:grid-cols-3 gap-3")(
        Div(
            ItemDetailCard(
                "Data Entry",
                *data_entry_card_content,
                card_id="modal-data-entry",
                col_span=1,
            ),
            ItemDetailCard(
                "Entities",
                Span("Click to load...", cls="italic"),
                card_id="modal-entities",
                col_span=1,
                start_collapsed=True,
                lazy_load_url=str(urls.URLS[Url.get_entities_element]).replace(
        ...
        Div(
            ItemDetailCard(
                "Item Info",
                *item_info_card_content,
                card_id="modal-item-info",
                col_span=1,
                start_collapsed=True,
            ),
            ItemDetailCard(
                "Text Details",
                *text_details_card_content,
                card_id="modal-text-details",
                col_span=1,
                start_collapsed=True,
            ),
            ItemDetailCard(
        ...
        Div(
            ItemDetailCard(
                ...
                *contact_info_card_content,
                card_id="modal-contact-info",
                col_span=1,
            ),
            ItemDetailCard(
                "Course Details (Osiris)",
                *course_details_card_content,
                card_id="modal-course-details",
                col_span=1,
                start_collapsed=True,
            ),
            ItemDetailCard(
                "Teachers/Persons (Osiris)",
                *teachers_card_content,
                card_id="modal-teachers",
                col_span=1,
                start_collapsed=True,
            ),
            cls="flex flex-col space-y-3",
        ...
        ItemDetailCard(
            "PDF Viewer",
            Span("Click to load PDF...", cls="italic"),
            card_id="modal-pdf-viewer",
            col_span=3,
            start_collapsed=True,
            lazy_load_url=str(urls.URLS[Url.get_pdf_element]).replace(
                "{material_id:int}", str(material_id)
            ),
        ),
        ItemDetailCard(
            "Extracted Text",
            Span("Click to load text...", cls="italic"),
            card_id="modal-extracted-text",
            col_span=3,
            start_collapsed=True,
            lazy_load_url=str(urls.URLS[Url.get_extracted_text_element]).replace(
                "{material_id:int}", str(material_id)
            ),
        ),
    )
    ...
    prev_btn_attrs = {
        "id": "modal-prev-btn",
        "cls": f"{ButtonT.secondary} btn-sm",
        "disabled": prev_item_id is None,
    }
    if prev_item_id:
        prev_btn_attrs.update(
            {
                "hx_get": str(urls.URLS[Url.show_item_details]).replace(
                    "{material_id:int}", str(prev_item_id)
                ),
    ...
    next_btn_attrs = {
        "id": "modal-next-btn",
        "cls": f"{ButtonT.secondary} btn-sm",
        "disabled": next_item_id is None,
    }
    if next_item_id:
        next_btn_attrs.update(
            {
                "hx_get": str(urls.URLS[Url.show_item_details]).replace(
                    "{material_id:int}", str(next_item_id)
                ),
                "hx_target": "#modal-placeholder",
                "hx_swap": "innerHTML",
            }
    ...
    modal_footer = Div(
        cls="modal-action mt-3 pt-3 border-t border-base-300"
    )(  # Reduced margins
    ...
    modal_box = Div(
        ...
        Div(  # Scrollable content area
            Form(
                Input(type="hidden", name="material_id", value=str(material_id)),
                modal_cards,
                id="modal-details-form",
                hx_post=str(urls.URLS[Url.save_item_details]),
                hx_include="[name='material_id'], [name='remarks']",  # Only send these for this form
                hx_target="body",
                hx_swap="none",  # Target body for toasts, swap none for form itself
    ...
    modal_backdrop_form = Form(method="dialog", cls="modal-backdrop")(
        NotStr("<button>close</button>")
    )

    # Return tuple of components for FastHTML, and HTMX headers to trigger modal opening
    return (modal_box, modal_backdrop_form), HtmxResponseHeaders(
        trigger="openModalEvent"
    ).headers  # type: ignore


@rt(dashboard_urls_module.URLS[Url.login])  # Login GET route
async def get_login_page() -> list[
    FT
]:  # Changed to async for consistency, though not strictly needed if no await
    ...
    login_form = Form(
        Div(
            fh.Label(
                "Email",
                html_for="email",
                cls="block text-gray-700 text-sm font-bold mb-2",
            ),  # Use fh.Label
            Input(
                id="email",
                type="email",
        ...
        Div(
            ...
            Input(
                ...
                type="password",
                placeholder="******************",
                cls="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline",
            ),
            cls="mb-6",
        ),
        fh.Button(
            "Login",
            cls="btn btn-primary bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-full",
            type="submit",
        ),  # Added w-full
        action=str(dashboard_urls_module.URLS[Url.login]),
        method="post",
        cls="bg-white shadow-xl rounded px-8 pt-6 pb-8 mb-4",  # Increased shadow
    )
    # Centered login card
    return [
        ...
        Div(
            Card(
                CardTitle(
                    H2(
                        "Copyright Dashboard Login",
                        cls="text-center text-2xl font-bold text-primary",
                    ),
                    cls="items-center justify-center pt-4",
                ),  # Centered title
                CardBody(login_form, cls="p-6"),  # Adjusted padding
                cls="w-full max-w-md bg-base-100 shadow-2xl rounded-lg",
            ),  # Added max-width and shadow
            cls="flex items-center justify-center min-h-screen bg-gradient-to-br from-primary to-secondary p-4",
        ),  # Fullscreen gradient
    ]
...
async def post_login_form(
    login_input: Login, session: dict[str, Any]
) -> RedirectResponse:  # login_input uses Pydantic binding from fasthtml
    """
    ...
    try:
        ...
        logger.error(
            f"Database error during login for user '{login_input.email}': {e_db_user}"
        )
    ...
    if user_data.get("pwd") == login_input.pwd:  # Insecure direct comparison
        ...
        if "app_state" in session:
            del session["app_state"]  # Clear any old app state
        add_toast(
            session,
            f"Login successful! Welcome, {user_data.get('name', 'User')}!",
            "success",
        )
        return RedirectResponse(
            str(dashboard_urls_module.URLS[Url.data_grid]), status_code=303
        )
    else:
        logger.warning(
            f"Login attempt failed: Incorrect password for '{login_input.email}'."
        )
        add_toast(session, "Invalid email or password.", "error")
        return LOGIN_REDIRECT_RESPONSE
...
async def logout_route(
    session: dict[str, Any],
) -> RedirectResponse:  # Changed to async for consistency
    """
...
async def root_redirect_to_data_grid() -> RedirectResponse:  # Changed to async
    ...
    return RedirectResponse(
        url=str(dashboard_urls_module.URLS[Url.data_grid]), status_code=302
    )
...
def _update_global_urls_from_router(
    router: QueryRouter, url_enum: type[Enum], target_dict: dict
) -> None:
    ...
    for member in url_enum:  # type: ignore
        ...
        try:
            ...
            route_func = (
                getattr(router.lookup_path(f"/{route_name}")[0], "endpoint", None)
                if router.lookup_path(f"/{route_name}")
                else None
            )
            if route_func and hasattr(route_func, "to"):
                target_dict[member] = route_func.to()  # type: ignore
            elif (
                route_name in router.paths
            ):  # Fallback if .to is not directly on endpoint
                target_dict[member] = router.paths[route_name]  # Store the path pattern
            else:
                logger.warning(
                    f"Could not find route path for Url enum member: {route_name}"
                )
        except Exception as e_url:  # pragma: no cover
            logger.warning(
                f"Error getting path for Url enum member {route_name}: {e_url}"
            )
...
def start_dashboard_server() -> None:  # Renamed 'start' to be more descriptive
    ...
    """
    logger.info(
        f"Starting dashboard server on port {APP_PORT}. Access at {APP_ROOT_URL}"
    )
    ...
    logger.info(
        "To run the dashboard, use the --dashboard flag with run.py, or run an ASGI server like Uvicorn (e.g., uvicorn easy_access.dashboard.dash:app --reload)"
    )
...
if __name__ == "__main__":  # pragma: no cover
    ...
    logger.info(
        "Dashboard module executed directly. Starting Uvicorn server for development."
    )
    ...
    logger.info(
        "To run the dashboard: uvicorn easy_access.dashboard.dash:app --reload --port PORT_NUMBER"
    )
