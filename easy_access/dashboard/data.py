...
from typing import Any  # Use specific Dict, List, Tuple
...
from easy_access.dashboard.web import (  # AppState for state management
    AppState,
    load_app_state,
)
...
class ProcessedDataResult:
    ...
    total_pages: int
    filter_counts: dict[str, dict[str, int | str]]
...
def process_state(
    ...
    initial_state_as_dict: dict[str, Any] = asdict(
        app_state
    )  # For detecting if state changed
    ...
    if action_param == "reset":
        if (
            app_state.filters or app_state.page != 1
            ...
            logger.debug(
                "Reset action requested, but no active filters or non-first page. No state change."
            )
    ...
    # Handle specific parameter changes if no 'reset' action
        ...
        if new_sort_by != app_state.sort_by or new_sort_desc != app_state.sort_desc:
            ...
            logger.debug(
                f"Sort changed to: by '{new_sort_by}', desc: {new_sort_desc}. Page reset."
            )
        ...
        is_checkbox_filter_change = (
            "checked" in request_params
        )  # Specific param from checkbox toggle
        ...
        for param_key, param_value in request_params.items():
            if param_key.startswith("filter_"):
                filter_field_name = param_key[
                    len("filter_") :
                ]  # e.g., "workflow_status"
                if filter_field_name not in FILTERABLE_COLUMNS:
                    logger.debug(
                        f"Skipping unknown filter key '{filter_field_name}' from request parameters."
                    )
                    continue

                current_filter_value_str = current_active_filters.get(
                    filter_field_name, ""
                )
                ...
                if is_checkbox_filter_change:
                    ...
                    temp_current_values_set = (
                        set(current_filter_value_str.split("|"))
                        if current_filter_value_str
                        else set()
                    )
                    if is_checked_bool:
                        temp_current_values_set.add(clicked_checkbox_value)
                    else:
                        temp_current_values_set.discard(clicked_checkbox_value)
                    temp_current_values_set.discard("")  # Clean up

                    simulated_new_value_str = (
                        "|".join(sorted(list(temp_current_values_set)))
                        if temp_current_values_set
                        else ""
                    )

                    if simulated_new_value_str != current_filter_value_str:
                        if app_state.apply_filter_change(
                            filter_field_name, clicked_checkbox_value, is_checked_bool
                        ):
                            filter_action_occurred = True
                    ...
                    if new_text_filter_value != current_filter_value_str:
                        if app_state.apply_text_filter(
                            filter_field_name, new_text_filter_value
                        ):
                            filter_action_occurred = True
                        break  # Process only one text filter change at a time

        if (
            filter_action_occurred and "per_page" not in request_params
        ):  # Avoid double page reset
...
async def fetch_data(
    app_state: AppState, session: dict[str, Any]
) -> ProcessedDataResult:
    """
    ...
    """
    logger.debug(
        f"Fetching data for AppState: Page {app_state.page}, Filters {app_state.filters}, Sort {app_state.sort_by} {app_state.sort_desc}"
    )

    auth_details: dict[str, Any] = session.get("auth", {})
    faculty_constraint: dict[str, str] | None = None
    user_faculty: str | None = auth_details.get("faculty")
    user_role: str | None = auth_details.get("role")
    ...
    all_filtered_df: pl.DataFrame = get_filtered_sorted_df(
    ...
    if validated_app_state.page != app_state.page:
        logger.info(
            f"Page number validated and changed from {app_state.page} to {validated_app_state.page} (Total pages: {total_pages})"
        )
    ...
    ui_filter_counts: dict[str, dict[str, int | str]] = calculate_counts_for_ui(
    ...
    current_page_df_slice: pl.DataFrame = all_filtered_df.slice(
        offset, validated_app_state.per_page
    )
    logger.debug(
        f"Data slice for page {validated_app_state.page}: {current_page_df_slice.height} rows."
    )
...
def calculate_counts_for_ui(
    app_state: AppState, faculty_constraint: dict[str, str] | None
    ...
    base_filtered_df_lazy = get_filtered_sorted_df(
        app_state, extra_constraints=faculty_constraint
    ).lazy()
    count_if_option_already_selected = (
        base_filtered_df_lazy.select(pl.len()).collect().item()
    )
    ...
    if not EMPTY_FILTERS:
        ...
        _update_empty_filters(
            temp_empty_state, faculty_constraint
        )  # Pass constraint here too

    current_active_filters: dict[str, str] = app_state.get_active_filters()
    calculated_counts: dict[str, dict[str, int | str]] = {}

    for filter_col_name in FILTERABLE_COLUMNS:
        if (
            filter_col_name not in get_item_df().columns
        ):  # Ensure column exists in base data
            continue

        options_for_column: dict[str, Any]  # From BADGE_STYLES
        if (
            filter_col_name == "manual_classification"
            and "classification" in BADGE_STYLES
        ):  # Special case
            ...
            logger.debug(
                f"No predefined badge styles for {filter_col_name}, counts might be incomplete for UI options."
            )
        ...
        current_selections_for_this_filter_key: set[str] = (
            set(current_active_filters.get(filter_col_name, "").split("|"))
            if current_active_filters.get(filter_col_name)
            else set()
        )

        for option_val_str in options_for_column:
            if option_val_str in EMPTY_FILTERS.get(filter_col_name, []):
                calculated_counts[filter_col_name][option_val_str] = (
                    "-"  # Globally no items for this option
                )
                continue
            ...
            if option_val_str in current_selections_for_this_filter_key:
                ...
                calculated_counts[filter_col_name][option_val_str] = (
                    count_if_option_already_selected  # Or "-"
                )
            else:
                ...
                temp_simulated_filters[filter_col_name] = "|".join(
                    sorted(list(new_filter_val_parts))
                )

                # Calculate count with this simulated filter state
                count = _apply_filters_for_count(
                    temp_simulated_filters, faculty_constraint
                )
...
def _update_empty_filters(
    app_state_for_empty_calc: AppState, faculty_constraint: dict[str, str] | None
) -> None:
    """
    ...
    base_df_for_empty_calc = get_filtered_sorted_df(
        app_state_for_empty_calc, extra_constraints=faculty_constraint
    )
    ...
    for filter_col_name in FILTERABLE_COLUMNS:
        ...
        if (
            ...
            options_for_column_keys = (
                base_df_for_empty_calc.get_column(filter_col_name)
                .unique()
                .drop_nulls()
                .cast(pl.Utf8)
                .to_list()
        ...
        for option_val in options_for_column_keys:
            ...
            if (
                base_df_for_empty_calc.is_empty()
            ):  # If base df is empty, all options are "empty"
            ...
            try:
                ...
                count_expr = _create_filter_expression(
                    base_df_for_empty_calc, temp_filter_for_option
                )
                option_count = (
                    base_df_for_empty_calc.lazy()
                    .filter(count_expr)
                    .select(pl.len())
                    .collect()
                    .item()
                ...
                logger.warning(
                    f"Error calculating empty filter status for {filter_col_name}={option_val}: {e}"
                )
...
def _create_filter_expression(
    ...
    for col_name, filter_value_str in user_filters.items():
        if col_name not in df_schema_provider.columns:
            logger.debug(
                f"Filter column '{col_name}' not in DataFrame schema. Skipping."
            )
            continue
        ...
        for or_val_part in or_value_parts:
            ...
            if not or_val_cleaned:
                continue

            # Specific handling for 'None' string in classification-like columns
            if (
                col_name in ["manual_classification", "classification"]
                and or_val_cleaned.lower() == "none"
            ):
            ...
            # Example for numeric (adapt if other numeric types or precise comparison needed)
                ...
                with contextlib.suppress(
                    ValueError
                ):  # Ignore if or_val_cleaned is not a valid number
                    per_column_or_expressions.append(
                        pl.col(col_name) == float(or_val_cleaned)
                    )
            # Add more type handling here if necessary

        if per_column_or_expressions:
            if len(per_column_or_expressions) > 1:
                filter_expressions_list.append(
                    pl.any_horizontal(per_column_or_expressions)
                )
            else:
    ...
    if extra_constraints:
        for col_name, constraint_val_str in extra_constraints.items():
            if col_name not in df_schema_provider.columns:
                logger.debug(
                    f"Constraint column '{col_name}' not in DataFrame schema. Skipping."
                )
                continue
            if constraint_val_str:  # Ensure constraint value is not empty
                ...
                filter_expressions_list.append(
                    ...
                    == str(constraint_val_str).lower()
                )

    if not filter_expressions_list:
        return pl.lit(
            True
        )  # No filters, return expression that evaluates to True for all rows
    return pl.all_horizontal(
        filter_expressions_list
    )  # Combine all filter parts with AND


def get_filtered_sorted_df(
    app_state: AppState, extra_constraints: dict[str, str] | None = None
) -> pl.DataFrame:
    """
    ...
    final_filter_expression = _create_filter_expression(
        base_df, user_active_filters, extra_constraints
    )
    ...
    try:
        ...
        logger.error(
            f"Error applying filters: {e_filter}. Returning empty DataFrame for this query.",
            exc_info=True,
        )
    ...
    if sort_by_col and sort_by_col in filtered_df.columns:
        try:
            filtered_df = filtered_df.sort(
                by=sort_by_col, descending=is_sort_desc, nulls_last=True
            )
        except Exception as e_sort:  # Catch potential Polars sort errors
            logger.warning(
                f"Sort warning on column '{sort_by_col}': {e_sort}. Proceeding without this sort."
            )
    elif sort_by_col:
        logger.warning(
            f"Sort column '{sort_by_col}' not found in filtered DataFrame. Skipping sort."
        )
...
def _apply_filters_for_count(
    filters_to_apply: dict[str, str], faculty_constraint: dict[str, str] | None = None
) -> int:
    """
    ...
    if faculty_constraint:
        combined_filters.update(
            faculty_constraint
        )  # faculty_constraint will overwrite if key exists

    filter_expr = _create_filter_expression(
        base_df_for_count, combined_filters
    )  # Pass combined
    try:
        count = (
            base_df_for_count.lazy()
            .filter(filter_expr)
            .select(pl.len())
            .collect()
            .item(0, 0)
        ...
        logger.error(
            f"Error applying filters for count calculation ({filters_to_apply}, {faculty_constraint}): {e}"
        )
        return 0


async def store_item_changes(
    items_to_update_data: list[
        dict[str, Any]
    ],  # Changed from single dict or list to always list
    ...
    current_items_df = get_item_df().filter(
        pl.col("material_id").cast(pl.Utf8).is_in(material_ids_to_update)
    )
    ...
    for item_changes_from_ui in items_to_update_data:
        ...
        current_item_rows = current_items_df.filter(
            pl.col("material_id").cast(pl.Utf8) == mat_id_ui
        )
        if current_item_rows.is_empty():
            logger.warning(
                f"Material ID {mat_id_ui} not found in current data for update. Skipping."
            )
            continue

        full_item_data_dict: dict[str, Any] = current_item_rows.to_dicts()[
            0
        ]  # Get first (and only) row as dict
        ...
        for key, new_value in item_changes_from_ui.items():
            ...
            if key in full_item_data_dict:
                if str(new_value) != str(
                    full_item_data_dict[key]
                ):  # Compare as strings for simplicity
                    logger.debug(
                        f"Updating {key} for {mat_id_ui}: '{full_item_data_dict[key]}' -> '{new_value}'"
                    )
                full_item_data_dict[key] = new_value
            else:  # Key from UI not in current DB data dict (new field or schema mismatch?)
                logger.debug(
                    f"Adding new key '{key}' with value '{new_value}' for {mat_id_ui} during update."
                )
                full_item_data_dict[key] = new_value
...
async def get_item_history_for_dashboard(material_id: int) -> list[dict[str, Any]]:
    ...
    history_records: list[ItemUpdate] = await retrieve_item_history(
        [material_id]
    )  # retrieve_item_history is async
        ...
        formatted_history.append(
            {
                ...
                "material_id_log": changes_dict.get(
                    "material_id", record.material_id
                ),  # From log or record
                "changes_summary": "; ".join(changed_fields_summary_list)
                if changed_fields_summary_list
                else "No specific field changes logged.",
                "full_details_json": json.dumps(
                    changes_dict, indent=2
                ),  # For raw view or debugging
            }
        )
    logger.info(
        f"Retrieved {len(formatted_history)} history entries for material_id {material_id}."
    )
