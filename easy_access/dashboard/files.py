...
from typing import List, Dict, Optional, Union # Consistent type hints
...
try:
    ...
    logger.warning("Could not import Entity from easy_access.classification.entity_recognition. Dashboard display of entities might be affected.")
...
class Entities:
    ...
    grouped_by_label: Optional[Dict[str, List[Entity]]] = field(init=False, default=None)

    def __init__(self, entities_data: List[Dict[str, Union[int, str, float, None]]]) -> None:
        """
    ...
    def get_grouped_and_sorted_entities(self) -> Tuple[Dict[str, List[Entity]], List[str]]:
        """
        ...
        def sort_key_for_labels(label: str) -> int:
            ...
            if "recognized" in label_lower: return 0
            if "copyright" in label_lower or "license" in label_lower: return 1
            if "university" in label_lower: return 2
            return 3 # Everything else

        sorted_label_keys = sorted(self.grouped_by_label.keys(), key=sort_key_for_labels)
...
def get_entities(material_id: int) -> Optional[Entities]:
    """
    ...
    Returns:
        Optional[Entities]: An Entities object if the file is found and parsed successfully,
                            None otherwise.
    """
    if not SETTINGS.dirs.get(DirSetting.PDF_DOWNLOADS) or not SETTINGS.dirs[DirSetting.PDF_DOWNLOADS].full.exists():
        logger.error(f"PDF_DOWNLOADS directory not configured or does not exist. Cannot get entities for {material_id}.")
        return None

    # Updated filename based on changes in entity_recognition.py
    entities_file_path = SETTINGS.dirs[DirSetting.PDF_DOWNLOADS].full / f"{material_id}_entities.json"

    if not entities_file_path.exists():
        logger.debug(f"Entities file not found for material_id {material_id} at {entities_file_path}")
        return None

    try:
        with open(entities_file_path, "r", encoding="utf-8") as f: # Specify read mode
            entities_data_list: List[Dict[str, Union[int, str, float, None]]] = json.load(f)
        return Entities(entities_data_list)
    except json.JSONDecodeError as e:
        logger.error(f"Error decoding JSON from entities file {entities_file_path}: {e}")
    except Exception as e: # Catch other potential errors like file read issues or Entities init error
...
def get_extracted_text(material_id: int) -> str:
    ...
    """
    if not SETTINGS.dirs.get(DirSetting.PDF_DOWNLOADS) or not SETTINGS.dirs[DirSetting.PDF_DOWNLOADS].full.exists():
        logger.error(f"PDF_DOWNLOADS directory not configured or does not exist. Cannot get text for {material_id}.")
    ...
    extracted_text_content: str = f"No extracted text file found for material ID {material_id}."
    found_file_path: Optional[Path] = None

    for suffix in preferred_suffixes:
        potential_file_path = SETTINGS.dirs[DirSetting.PDF_DOWNLOADS].full / f"{material_id}{suffix}"
        if potential_file_path.exists():
            found_file_path = potential_file_path
            break # Found the highest priority file

    if found_file_path:
        logger.info(f"Retrieving extracted text for {material_id} from {found_file_path.name}")
        try:
            with open(found_file_path, "r", encoding="utf-8") as f: # Specify read mode
        ...
        logger.info(f"No text file found for material ID {material_id} with any of the expected suffixes.")
