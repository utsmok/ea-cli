...
from typing import Any, Dict, List, Optional, Set, Tuple # Added Set, Union, Tuple
...
from easy_access.dashboard.constants import DEFAULT_PER_PAGE, FILTERABLE_COLUMNS, db # db from constants
...
class AppState:
    ...
    filters: Dict[str, str] = field(default_factory=dict) # Stores active filters as {col_name: "val1|val2"}
    ...
    def set_sort(self, sort_by_col: Optional[str], is_descending: bool) -> None:
    ...
    def apply_filter_change(self, filter_key: str, toggled_value: str, is_checked: bool) -> bool:
        """
        ...
        current_values_set: Set[str] = set(current_filter_values_str.split("|")) if current_filter_values_str else set()
    ...
    def filter_params_dict(self) -> Dict[str, str]:
        ...
        if not self.filters:
            return {}
        return {f"filter_{k}": v for k, v in self.filters.items() if v} # Ensure only non-empty filter values
...
def load_app_state(session: Dict[str, Any]) -> AppState:
    ...
    """
    app_state_from_session: Optional[Dict[str, Any]] = session.get("app_state")
    if isinstance(app_state_from_session, dict):
        try:
            ...
            filtered_state_dict = {k: v for k,v in app_state_from_session.items() if k in valid_keys}
            state = AppState(**filtered_state_dict)
            logger.debug("AppState loaded from session.")
            return state
        except (TypeError, ValueError) as e: # Catch errors if dict structure doesn't match AppState
            logger.warning(f"Invalid 'app_state' structure in session: {e}. Using default AppState.")
    else:
        if app_state_from_session is not None: # Log if it exists but isn't a dict
             logger.warning(f"Found 'app_state' in session but it's not a dictionary (type: {type(app_state_from_session)}). Using default AppState.")
        else: # Not in session
...
def create_users_from_secrets() -> None:
    ...
    users_secret_file_path: Path = SETTINGS.dirs[DirSetting.SCRIPT_DATA].full / USERS_FILENAME

    if not users_secret_file_path.exists():
        logger.error(f"Users secret file '{USERS_FILENAME}' not found at {users_secret_file_path}. Cannot create/update users.")
        return

    try:
        with open(users_secret_file_path, "r", encoding="utf-8") as f:
            users_data_from_file: Dict[str, Dict[str, str]] = json.load(f)
    except (json.JSONDecodeError, OSError) as e:
        logger.error(f"Error reading or parsing users secret file {users_secret_file_path}: {e}")
        return

    try:
        existing_users_emails_q = db.q(f"SELECT email FROM {users.tname}") # Use users.tname for table name
        existing_users_emails: List[str] = [user_row.get("email") for user_row in existing_users_emails_q if user_row] # type: ignore
    ...
    for email, data in users_data_from_file.items():
        if email not in existing_users_emails:
            try:
                ...
                users.insert(
                    name=data.get("name"), email=email, pwd=data.get("password"), # PWD should be hashed
                    faculty=data.get("faculty"), role=data.get("role")
                )
                users_added_count += 1
    ...
    if users_added_count > 0:
        logger.info(f"Synchronized users from secrets file. Added {users_added_count} new users.")
    else:
...
if users.tname not in db.tnames: # Check if table name exists in db's list of table names
    logger.info(f"Table '{users.tname}' not found, creating it.")
    users.create(
        email=str, name=str, pwd=str, # PWD should store hash
        faculty=str, role=str, pk="email"
    )
else:
...
def auth_middleware_before_request(request: Request, session: Dict[str, Any]) -> Optional[RedirectResponse]:
    """
    ...
    if not request.scope["auth"]: # If 'auth' details are not in session
        logger.debug(f"User not authenticated for path: {request.url.path}. Redirecting to login.")
        return LOGIN_REDIRECT_RESPONSE
    logger.debug(f"User authenticated: {request.scope['auth'].get('email')}. Allowing access to {request.url.path}")
...
def _register_static_file_extensions() -> None:
    ...
    from fasthtml.xtend import reg_re_param # Local import as it's a specific setup call
    ...
    try:
        ...
        logger.debug(f"Registered common static file extensions for fasthtml: {common_static_exts}")
    except Exception as e_reg: # pragma: no cover
        logger.warning(f"Could not register static file extensions with fasthtml: {e_reg}")
