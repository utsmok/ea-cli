...
async def load_osiris_data() -> None:
    ...
    if not osiris_data_file.exists:
        logger.warning(f"{osiris_data_file.path} not found; Osiris course data will not be loaded to DB.")
        return
    ...
    try:
        existing_course_codes_query = await Course.all().values_list("cursuscode", flat=True)
        ...
        for course_data in osiris_data_dict.values():
            ...
            if not cursuscode_str or not str(cursuscode_str).isdigit():
                logger.debug(f"Skipping Osiris course data due to missing or invalid cursuscode: {course_data.get('name')}")
                continue
            ...
            course_dict: dict[str, Any] = {
                ...
                "internal_id": int(course_data["internal_id"]) if course_data.get("internal_id") and str(course_data.get("internal_id")).isdigit() else None,
                "name": course_data.get("name"),
                "short_name": course_data.get("short_name"),
                "ec": int(round(float(ec_str))) if ec_str and ec_str.replace('.', '', 1).isdigit() else None,
            ...
            if faculty_abbr:
                ...
                if faculty:
                    ...
                    logger.debug(f"Faculty '{faculty_abbr}' not found for course '{course_dict['name']}'.")

            course_dicts_to_create.append(course_dict)
            existing_course_codes.add(cursuscode) # Add to set to prevent re-processing if duplicated in source JSON

        if course_dicts_to_create:
            await Course.bulk_create(objects=[Course(**c) for c in course_dicts_to_create])
            logger.info(f"Created {len(course_dicts_to_create)} new courses from Osiris data.")
        else:
...
async def load_org_data_from_settings() -> None:
    ...
    await init_tortoise()
    try:
        faculties_from_settings: list[SettingsFaculty] = SETTINGS.university_settings.faculties
        ...
        for faculty_setting in faculties_from_settings:
            ...
            existing_programmes_q = await Programme.filter(faculty=faculty_obj).all().values_list("name", "abbreviation")
            existing_programmes_set: set[tuple[str, str | None]] = {(name, abbr) for name, abbr in existing_programmes_q} # type: ignore

            for programme_setting in faculty_setting.programmes:
                if not programme_setting.name: continue # Skip if no name

                prog_abbr = programme_setting.abbreviation
                if not prog_abbr: # Attempt to generate abbreviation if missing
                    abbr_parts: list[str] = []
                    if programme_setting.programme_type == "b": abbr_parts.append("B")
                    elif programme_setting.programme_type == "m": abbr_parts.append("M")
                    else: abbr_parts.append("O") # Other/Unknown

                    name_parts = programme_setting.name.lower().replace("bachelor", "").replace("master", "").strip().split(" ")
                    abbr_parts.extend([part[0] for part in name_parts if part][:2]) # First letter of first two words
            ...
            if programmes_to_create:
                await Programme.bulk_create(objects=[Programme(**p) for p in programmes_to_create])
                logger.info(f"Created {len(programmes_to_create)} programmes for faculty {faculty_setting.abbreviation}.")
...
async def load_person_data() -> None:
    ...
    if not person_data_file.exists:
        logger.warning(f"{person_data_file.path} not found; person data will not be loaded.")
        return
    ...
    try:
        existing_person_input_names_q = await Person.all().values_list("input_name", flat=True)
        existing_person_input_names: set[str] = set(existing_person_input_names_q) # type: ignore

        university_org = await Organization.get_or_none(abbreviation="UT", hierarchy_level=0)
        if not university_org:
            logger.error("University 'UT' root organization not found. Cannot properly set parent organizations.")
        ...
        for person_data_dict in persons_json_data:
            ...
            person_model_data: dict[str, Any] = {
                ...
                "match_confidence": float(person_data_dict["match_confidence"]) if person_data_dict.get("match_confidence") is not None else None,
                "first_name": str(person_data_dict.get("other_names", [None])[0] or "").strip() or None, # Assuming first item is first name
                "email": str(person_data_dict.get("email", "")).strip() or None,
                "faculty": faculty_obj,
                "people_page_url": str(person_data_dict.get("people_page_url", "")).strip() or None,
            }
            ...
            if isinstance(org_data_list, list):
                org_data_list.sort(key=lambda x: x.get("abbr", "").count("-")) # Process parents first

                for org_dict_item in org_data_list:
                    if not isinstance(org_dict_item, dict): continue
                    try:
                        org_name = str(org_dict_item.get("name", "")).strip()
                        org_full_abbr = str(org_dict_item.get("abbr", "")).strip()
                        if not org_name or not org_full_abbr: continue

                        org_sole_abbr = org_full_abbr.split("-")[-1].strip()
                        hierarchy_level = org_full_abbr.count("-") + 1 # Level 1 for "FAC", 2 for "FAC-DEPT"

                        org_model_data: dict[str, Any] = {
                            "name": org_name, "abbreviation": org_sole_abbr,
                            "full_abbreviation": org_full_abbr, "hierarchy_level": hierarchy_level,
                        }

                        # Determine parent organization
                        parent_org_obj: Organization | None = None
                        if hierarchy_level == 1 and university_org: # Faculty, parent is University
                            parent_org_obj = university_org
                        elif hierarchy_level > 1:
                            parent_full_abbr = org_full_abbr.rsplit("-", 1)[0].strip()
                            parent_org_obj = await Organization.get_or_none(full_abbreviation=parent_full_abbr)
                        ...
                        logger.warning(f"Error processing organization {org_dict_item} for person {input_name}: {e_org}")
...
async def load_linked_persons_for_courses() -> Counter[str]:
    ...
    if not osiris_data_file.exists:
        logger.warning(f"{osiris_data_file.path} not found; cannot link persons to courses.")
        return counter
    ...
    try:
        for course_data_item in osiris_data_dict.values(): # Renamed course_data to course_data_item
            cursuscode_str = course_data_item.get("cursuscode")
            if not cursuscode_str or not str(cursuscode_str).isdigit(): continue
            ...
            roles_to_process: list[tuple[str, str]] = [ # (role_name_in_json, role_name_in_db)
                ("teachers", "teacher"), ("contacts", "contact"), ("tutors", "tutor"),
                ("docenten", "docent"), # Assuming 'docenten' is another list of contacts/teachers
                ("examinators", "examinator"), ("unknown_role", "unknown_role")
            ]
            ...
            for json_role_key, db_role_name in roles_to_process:
                ...
                if isinstance(person_names_list, list):
                    for person_name in person_names_list:
                        if not isinstance(person_name, str): continue # Skip if name is not string
                        person_obj = await Person.get_or_none(input_name=person_name.strip())
                        if person_obj:
                            _, created = await CourseEmployee.get_or_create(
                                course=course_obj, person=person_obj, role=db_role_name
                            )
                            if created: counter[db_role_name] += 1
                        else:
                            logger.debug(f"Person '{person_name}' not found for course {cursuscode_str}, role '{db_role_name}'.")
...
async def load_base_data() -> None:
    ...
    """
    logger.info("Starting to load all base data (organizations, courses, persons, links)...")
    ...
    try:
        ...
        counts_before: dict[str, int] = {
            "Faculty": await Faculty.all().count(), "Programme": await Programme.all().count(),
            "Course": await Course.all().count(), "Person": await Person.all().count(),
            "Organization": await Organization.all().count(), "MissingCourse": await MissingCourse.all().count(),
...
async def load_raw_copyright_data(source_data: File | pl.DataFrame) -> None: # Removed None option, require data
    """
    ...
    def _read_and_prep_excel(file_obj: File) -> pl.DataFrame:
        ...
        try:
            latest_file_date_str = file_obj.created.strftime("%Y-%m-%d") # Assuming File obj has 'created'
            ...
            transformed_df = renamed_df.with_columns(
                ...
                pl.lit(WorkflowStatus.ToDo.value).alias("workflow_status"), # Default workflow
                pl.col("last_change").str.replace(r"^- ভারতবর্ষ$", None) # Example of specific cleaning if needed
            ...
            logger.error(f"Error reading or prepping Excel file {file_obj.name}: {e_read}")
    ...
    try:
        logger.info(f"Items in DB before loading raw copyright data: {await CopyrightItem.all().count()}")
        ...
        if isinstance(source_data, File):
            ...
            logger.info(f"Loading {len(source_data)} raw copyright items from provided DataFrame.")
            df_to_process = source_data.clone() # Use a copy
        else:
            logger.error(f"Unsupported data type for load_raw_copyright_data: {type(source_data)}")
        ...
        existing_mat_ids_query = await CopyrightItem.all().values_list("material_id", flat=True)
        existing_mat_ids: set[int] = set(existing_mat_ids_query) # type: ignore
        logger.info(f"Read {len(item_dicts_for_processing)} standardized items. {len(existing_mat_ids)} items already in DB.")

        new_item_orm_list: list[CopyrightItem] = []
        items_for_update_list: list[dict[str, Any]] = [] # List of dicts for update_copyright_items
        ...
        for item_dict_idx, item_data_dict in enumerate(item_dicts_for_processing):
            ...
            if material_id_val is None or not str(material_id_val).isdigit():
                logger.warning(f"Skipping item due to missing/invalid material_id: {item_data_dict.get('filename', 'N/A')}")
                continue
            ...
            if material_id in existing_mat_ids:
                ...
                created_orm_item = await copyright_item_from_dict(item_data_dict) # This converts types
                if created_orm_item:
                    new_item_orm_list.append(created_orm_item)
                else:
                    logger.warning(f"Failed to convert item dict to ORM object: {material_id}")

            if (item_dict_idx + 1) % 100 == 0:
                logger.debug(f"Processed {item_dict_idx + 1}/{len(item_dicts_for_processing)} raw items for DB ingestion.")
        ...
        logger.info(f"Total items in DB after creating new ones: {item_count_after_new}")

        if items_for_update_list:
            logger.info(f"Found {len(items_for_update_list)} existing items from raw data to check for updates.")
            ...
            await update_copyright_items(items_for_update_list, overwrite=True, update_relations=False)
        ...
        if new_item_orm_list or items_for_update_list:
            logger.info("Updating LLM classifications and copyright relations after raw data load.")
            await load_llm_classifications() # This should handle its own init/close
            await update_copyright_relations() # This should also handle its own init/close
        ...
        await Tortoise.close_connections()
        if process_error and not (new_item_orm_list or items_for_update_list): # Only re-raise if nothing was processed
             raise process_error
...
async def load_llm_classifications() -> None:
    ...
    await init_tortoise()
    try:
        existing_class_mat_ids_q = await LLMClassification.all().values_list("used_material_id", flat=True)
        existing_class_mat_ids: set[int] = set(existing_class_mat_ids_q) # type: ignore
        logger.info(f"Found {len(existing_class_mat_ids)} existing LLM classifications in DB.")

        classifications_dir: Directory = SETTINGS.dirs[DirSetting.CLASSIFICATIONS]
        all_json_files: list[File] = [f for f in classifications_dir.files if f.name.endswith(".json")]
        ...
        if old_files_to_delete:
            logger.info(f"Found {len(old_files_to_delete)} old LLM JSON files. Removing...")
            for old_f in old_files_to_delete: old_f.delete()

        # Re-list files after potential deletion
        current_json_files: list[File] = [f for f in classifications_dir.files if f.name.endswith(".json")]
        ...
        for json_file in current_json_files:
            try:
                ...
                if not material_id_str.isdigit():
                    logger.warning(f"Skipping JSON file with non-numeric name: {json_file.name}")
                    continue
                ...
                data_dict["used_material_id"] = material_id # Ensure this key is present
                ...
                logger.warning(f"Invalid JSON in file {json_file.name}. Marking for deletion.")
                files_to_delete_after_processing.append(json_file)
            except Exception as e_file:
                logger.warning(f"Error processing LLM JSON file {json_file.name}: {e_file}")
                files_to_delete_after_processing.append(json_file) # Mark for deletion on other errors too

        logger.info(f"Retrieved {len(new_classifications_data)} new LLM classifications from JSON files.")
        for f_del in files_to_delete_after_processing:
            if f_del in new_classifications_data: # Ensure we don't try to use it if it's marked for deletion
                new_classifications_data.remove(f_del) # This comparison is wrong, need to remove by mat_id or filename
        ...
        if new_classifications_data:
            ...
            for classification_dict in new_classifications_data:
                ...
                try:
                    ...
                    list_fields = ["author_names", "doi", "isbn", "source_url", "license", "topic"]
                    for field_name in list_fields:
                        if field_name in classification_dict and isinstance(classification_dict[field_name], str):
                            try:
                                potential_list = json.loads(classification_dict[field_name])
                                if isinstance(potential_list, list):
                                    classification_dict[field_name] = potential_list
                                else: # Not a list, keep as string or wrap in list
                                     classification_dict[field_name] = [str(classification_dict[field_name])]
                            except json.JSONDecodeError: # Not a JSON list string
                                classification_dict[field_name] = [str(classification_dict[field_name])] if classification_dict[field_name] else []
                        elif field_name in classification_dict and not isinstance(classification_dict[field_name], list):
                             classification_dict[field_name] = [str(classification_dict[field_name])] if classification_dict[field_name] else []


                    orm_objects_to_create.append(LLMClassification(**classification_dict))
                except Exception as e_orm:
                    logger.warning(f"Error creating LLMClassification ORM object from dict {classification_dict.get('used_material_id')}: {e_orm}")

            if orm_objects_to_create:
                await LLMClassification.bulk_create(orm_objects_to_create)
                logger.info(f"Created {len(orm_objects_to_create)} new LLM classifications in DB.")
        else:
...
async def load_pdfs() -> None:
    ...
    try:
        ...
        if not pdfs_dir.exists:
            logger.warning(f"PDF downloads directory {pdfs_dir.full} not found. Cannot load PDF metadata.")
            return

        all_pdf_files_on_disk: list[File] = [f for f in pdfs_dir.files_r if f.name.endswith(".pdf") and not f.name.startswith("~$")]
        ...
        for pdf_file in all_pdf_files_on_disk:
            ...
            name_part = pdf_file.name.split("_")[0] if "_" in pdf_file.name else pdf_file.name.split(".")[0]
            if name_part.isdigit():
                pdf_files_by_mat_id[int(name_part)] = pdf_file
            else:
                logger.debug(f"Could not extract material_id from PDF filename: {pdf_file.name}")

        if not pdf_files_by_mat_id:
            logger.info("No PDF files found with extractable material_ids in their names.")
            return

        existing_pdf_db_mat_ids_q = await PDF.all().values_list("material_id", flat=True)
        ...
        for mat_id, pdf_file_obj in pdf_files_by_mat_id.items():
            ...
            if not related_item:
                logger.debug(f"No CopyrightItem found for material_id {mat_id} (PDF: {pdf_file_obj.name}). Skipping PDF metadata.")
                continue
        ...
        if pdf_metadata_to_create:
            await PDF.bulk_create(objects=[PDF(**p_dict) for p_dict in pdf_metadata_to_create])
            logger.info(f"Created {len(pdf_metadata_to_create)} new PDF metadata records in DB.")
        else:
            logger.info("No new PDF metadata records to create (either exist or no related CopyrightItem).")
