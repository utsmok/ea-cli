...
from enum import StrEnum # Enum was already imported, StrEnum is more specific if used
from typing import Any

import polars as pl
from tortoise import Tortoise

from easy_access.db.base import copyright_item_from_dict, init as init_tortoise # Renamed init for clarity
...
from easy_access.utils import determine_course_code # Assuming this is still the location
...
async def link_llm_classifications_to_copyright_items() -> None:
    ...
    await init_tortoise()
    try:
        classifications_to_link = await LLMClassification.filter(item__isnull=True).all()
        linked_count = 0
        for classification in classifications_to_link:
            try:
                ...
                if item:
                    ...
                    await item.save(update_fields=['llm_classification_id', 'modified_at']) # Be specific
                    linked_count += 1
        ...
        logger.info(f"Successfully linked {linked_count} of {len(classifications_to_link)} LLM classifications.")
    except Exception as e:
        logger.error(f"General error in link_llm_classifications_to_copyright_items: {e}")
...
async def link_courses_to_copyright_items() -> None:
    ...
    await init_tortoise()
    try:
        items_w_prefetch = await CopyrightItem.all() # Consider prefetching faculty if used by determine_course_code indirectly
        logger.info(f"Retrieved {len(items_w_prefetch)} copyright items for course linking.")

        links_added: int = 0
        course_codes_processed: int = 0 # Tracks how many codes were attempted, not just found
        ...
        for item in items_w_prefetch:
            ...
            course_codes_str: set[str] = determine_course_code(item.course_code, item.course_name)
            ...
            for code_str in course_codes_str:
                ...
                if not code_str or not code_str.isdigit(): # Skip if empty or not purely numeric
                    logger.debug(f"Invalid course code format '{code_str}' for item {item.material_id}.")
                    continue
                try:
                    ...
                    if course:
                        ...
                        links_added += 1
                        logger.debug(f"Linked item {item.material_id} to course {cursuscode}.")
                    else:
                        logger.debug(f"Course with code {cursuscode} not found for item {item.material_id}.")
                except ValueError: # For int conversion
                     logger.warning(f"Course code '{code_str}' is not a valid integer for item {item.material_id}.")
                except Exception as e:
                    logger.warning(
                        f"Error linking course {code_str} to item {item.material_id}: {e}"
                    )
        logger.info(f"Attempted linking for {course_codes_processed} course codes. Successfully added/confirmed {links_added} links.")
    except Exception as e:
...
async def update_duplicate_status() -> None:
    ...
    try:
        ...
        for item in items:
            ...
            pdf_record = await PDF.get_or_none(material_id=item.material_id) # Assumes one-to-one or primary PDF
            if pdf_record and pdf_record.replace_with_id: # Check replace_with_id which is the FK field
                # The related PDF this one should be replaced with
                replacing_pdf = await PDF.get_or_none(material_id=pdf_record.replace_with_id)
                if replacing_pdf:
                    item.is_duplicate = True
                    item.replacement_id = replacing_pdf.material_id # Store the material_id of the PDF it's replaced by

            if item.is_duplicate != original_is_duplicate or item.replacement_id != original_replacement_id:
                await item.save(update_fields=["is_duplicate", "replacement_id", "modified_at"])
...
async def update_copyright_relations() -> None:
    ...
    try:
        ...
        await Tortoise.close_connections() # Ensure connection is closed at the end of the orchestration
...
class DataSource(StrEnum):
    ...
    EA_SCRIPT = "ea_script"             # Data generated or modified by the Easy Access script itself.
...
async def update_copyright_items(
    ...
    def _log_change(
        item_changes: dict[str, Any], field: str, new_value: Any, old_value: Any, reason: str
    ) -> dict[str, Any]:
        """Helper to log a field change and update the database item object."""
        logger.debug(f"Item {item_changes.get('material_id')}: [{reason}] Field '{field}': '{old_value}' -> '{new_value}'")
    ...
    def _compare_and_apply_field_changes(
    ...
    ) -> tuple[dict[str, Any], bool]: # Returns updated change log and a flag if db_item_obj was modified
        ...
        for field, priority_order in fields_to_compare.items():
            ...
            if isinstance(old_value, datetime) and isinstance(new_value, str):
                try: new_value = datetime.strptime(new_value.split(" ")[0], "%Y-%m-%d").replace(tzinfo=UTC)
                except ValueError: new_value = None # Or handle error
                if old_value: old_value = old_value.replace(tzinfo=UTC) # Ensure timezone consistency

            if isinstance(old_value, Enum): old_value = old_value.value
            if isinstance(new_value, Enum): new_value = new_value.value # Ensure new_value from dict is also compared as value if it's an Enum

            if new_value is None: continue # Skip if new value is None (don't overwrite with None unless intended)
            if new_value == old_value: continue

            if old_value is None or old_value == "": # DB value is empty/None, new value is not
                item_change_log = _log_change(item_change_log, field, new_value, old_value, "DB value empty, using new value")
                setattr(db_item_obj, field, new_value)
                item_modified_flag = True
            elif priority_order: # List-based priority
                new_rank = priority_order.index(new_value) if new_value in priority_order else float('inf')
                old_rank = priority_order.index(old_value) if old_value in priority_order else float('inf')
                if new_rank < old_rank: # Lower index means higher priority
                    item_change_log = _log_change(item_change_log, field, new_value, old_value, "New value has higher priority rank")
                    setattr(db_item_obj, field, new_value)
                    item_modified_flag = True
            elif isinstance(new_value, str) and isinstance(old_value, str): # String length heuristic (e.g. for remarks)
                if len(new_value.strip()) > len(old_value.strip()):
                    item_change_log = _log_change(item_change_log, field, new_value, old_value, "New string value longer")
                    setattr(db_item_obj, field, new_value)
                    item_modified_flag = True
            ...
            elif new_value != old_value : # Generic overwrite if different and no specific rule matched
                 item_change_log = _log_change(item_change_log, field, new_value, old_value, "Generic update, new value different")
                 setattr(db_item_obj, field, new_value)
                 item_modified_flag = True
    ...
    added_fields: dict[str, list[Any] | None] = {
        "workflow_status": [ws.value for ws in WorkflowStatus], # Highest priority first
    ...
    changeable_fields: dict[str, list[Any] | None] = {
        "manual_classification": [c.value for c in Classification], # Highest priority first
        "manual_identifier": None, "remarks": None, "scope": None,
        # Fields also updated by raw Qlik data, Qlik is source of truth for these
        "status": [s.value for s in Status], "last_change": None,
        "pages_x_students": None, "count_students_registered": None,
    }


    logger.info(f"Received {len(data)} items for update process. Overwrite: {overwrite}")
    ...
    if isinstance(data, pl.DataFrame):
        ...
        standardized_df = standardize_dataframe(data.clone()) # Clone to avoid modifying original

        all_input_ids_str = standardized_df.select(pl.col("material_id").cast(pl.Utf8)).to_series().to_list()
        existing_db_items = await CopyrightItem.filter(material_id__in=all_input_ids_str).all()
        existing_db_ids = {str(item.material_id) for item in existing_db_items}

        items_to_update_from_input = standardized_df.filter(pl.col("material_id").is_in(existing_db_ids)).to_dicts()
        new_item_dicts = standardized_df.filter(~pl.col("material_id").is_in(existing_db_ids)).to_dicts()
        ...
        all_input_ids = [str(item.get("material_id")) for item in data if item.get("material_id")]
        existing_db_items = await CopyrightItem.filter(material_id__in=all_input_ids).all()
        existing_db_ids = {str(item.material_id) for item in existing_db_items}

        items_to_update_from_input = [item for item in data if str(item.get("material_id")) in existing_db_ids]
        new_item_dicts = [item for item in data if str(item.get("material_id")) not in existing_db_ids]
    else:
    ...
    logger.info(f"# of existing items to check for updates: {len(items_to_update_from_input)}")

    created_orm_items: list[CopyrightItem] = []
    if new_item_dicts:
        for item_dict in new_item_dicts:
            orm_item = await copyright_item_from_dict(item_dict) # This handles type conversion
            if orm_item:
                created_orm_items.append(orm_item)
        if created_orm_items:
            try:
                await CopyrightItem.bulk_create(created_orm_items)
                logger.info(f"Bulk created {len(created_orm_items)} new copyright items.")
            except Exception as e:
                logger.warning(f"Error during bulk_create of new items: {e}. Attempting individual save.")
                for orm_item in created_orm_items:
                    try: await orm_item.save()
                    except Exception as ie: logger.error(f"Error saving new item {orm_item.material_id}: {ie}")
    ...
    for new_item_data_dict in items_to_update_from_input:
        ...
        if not db_item_instance: # Should not happen due to pre-filtering, but as a safeguard
            logger.warning(f"Item with material_id {mat_id_str} not found in DB map for update. Skipping.")
            continue
        ...
        if overwrite:
            ...
            for field_key in list(changeable_fields.keys()) + list(added_fields.keys()):
                if field_key in new_item_data_dict:
                    ...
                    if isinstance(old_db_value, Enum): old_db_value = old_db_value.value
                    if isinstance(new_value_from_input, Enum): new_value_from_input = new_value_from_input.value
                    if isinstance(old_db_value, datetime) and isinstance(new_value_from_input, str) :
                        try: new_value_from_input = datetime.strptime(new_value_from_input.split(" ")[0], "%Y-%m-%d")
                        except ValueError: pass # Keep as string if not parsable here, model field will handle

                    if str(new_value_from_input) != str(old_db_value): # Simple string comparison after basic normalization
                        item_change_log_details = _log_change(item_change_log_details, field_key, new_value_from_input, old_db_value, "[overwrite]")
                        setattr(db_item_instance, field_key, new_value_from_input) # Apply change to ORM object
                        item_was_modified_in_db = True
        else: # Heuristic comparison
            item_change_log_details, field_changed1 = _compare_and_apply_field_changes(new_item_data_dict, db_item_instance, added_fields, item_change_log_details)
            item_change_log_details, field_changed2 = _compare_and_apply_field_changes(new_item_data_dict, db_item_instance, changeable_fields, item_change_log_details)
        ...
        if item_was_modified_in_db:
            ...
            item_update_logs.append(ItemUpdate(change_details=item_change_log_details, material_id=db_item_instance.material_id)) # type: ignore
    ...
    if db_items_to_bulk_update:
        ...
        for log_entry_details in item_update_logs:
            for key in log_entry_details.change_details.keys(): # Iterate over keys in change_details dict
        ...
        if "modified_at" not in update_fields_list: # Always include modified_at if any changes occurred
            update_fields_list.append("modified_at")

        if not update_fields_list or (len(update_fields_list) == 1 and update_fields_list[0] == "modified_at" and not all_changed_field_keys):
             logger.info("No substantive fields to update in bulk apart from modified_at, or list is empty.")
        else:
            logger.info(f"Bulk updating {len(db_items_to_bulk_update)} items for fields: {update_fields_list}.")
            await CopyrightItem.bulk_update(db_items_to_bulk_update, fields=update_fields_list)
        ...
        if item_update_logs:
            ...
            for db_item_updated in db_items_to_bulk_update: # Iterate over actual updated ORM objects
                # Fetch the most recent ItemUpdate for this material_id (the one just created)
                latest_log_entry = await ItemUpdate.filter(material_id=db_item_updated.material_id).order_by("-created_at").first()
                if latest_log_entry:
    ...
    if (created_orm_items or db_items_to_bulk_update) and update_relations:
        ...
        await update_copyright_relations() # This function should also handle its own Tortoise connection
