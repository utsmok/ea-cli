...
import os # Keep for os.getcwd() if not fully replaced by Path.cwd() yet
import sys
from dataclasses import dataclass, field
from enum import Enum
from pathlib import Path
from typing import Any, Literal, TypeAlias # Added TypeAlias for older Python if needed
...
class SheetSetting(Enum):
    ...
    COMPLETE_DATA_COLS = "complete_data_cols" # Note: Seems unused in current parsing logic
...
class DataSettings:
    ...
    complete_data_cols: list[ColInfo] = field(default_factory=list, init=False) # Currently not explicitly parsed
    ...
    new_fields: dict[str, dict[str, Any]] = field(
...
class SettingsProgramme:
    ...
    programme_type: Literal["b", "m", "o"] = "o" # b=bachelor, m=master, o=other/unknown
    cluster: str | None = None
    faculty_name: str | None = None # Populated by UniversitySettings.make_programme_set
    faculty_abbreviation: str | None = None # Populated by UniversitySettings.make_programme_set
...
class UniversitySettings:
    ...
    def department_mapping(self) -> dict[str, str]:
        ...
        mapping_data: dict[str, str] = {
            f"{programme.abbreviation + ': ' if programme.abbreviation else ''}{programme.name}": programme.faculty_abbreviation or ""
            for programme in self.programmes if programme.name # Ensure programme.name is not None
        }
    ...
    def course_mapping(self) -> dict[str, dict[str, str]]:
        ...
        for faculty in self.faculties:
            if not faculty.abbreviation: continue # Skip faculty if no abbreviation
            ...
            if faculty_programme_map: # Only add faculty if it has programmes with clusters
...
class Settings:
    ...
    classification_options: list[str] = field(default_factory=list, init=False) # Parsed under 'unsorted'
    ...
    def load(self) -> None:
        ...
        try:
            ...
            std_logger.error(f"Settings file not found: {self.settings_file.path}. Default settings will be used.")
            self.raw_settings = {}
        except yaml.YAMLError as e:
            std_logger.error(f"Error parsing YAML settings from {self.settings_file.path}: {e}")
            self.raw_settings = {}
        except Exception as e:
            std_logger.error(f"Unexpected error loading settings from {self.settings_file.path}: {e}")
            self.raw_settings = {}
    ...
    def parse_settings(self) -> None:
        ...
        if not isinstance(self.raw_settings, dict): # Ensure raw_settings is a dict
            std_logger.error(f"Failed to load settings as a dictionary. Raw settings type: {type(self.raw_settings)}")
            return
        ...
        for key, value in self.raw_settings.items():
            ...
            if parser_method:
                try:
                    ...
                    std_logger.error(f"Error parsing settings section '{key}': {e}. Skipping this section.")
            else:
                std_logger.warning(f"Unrecognized settings key: '{key}'. Value will be set as a direct attribute if not handled by 'unsorted'.")
        ...
        if "unsorted" in self.raw_settings and isinstance(self.raw_settings["unsorted"], dict):
    ...
    def parse_university(self, value: dict[str, Any]) -> None:
        ...
        for faculty_data in value.get("faculties", []):
            if not isinstance(faculty_data, dict): continue
            ...
            if isinstance(programmes_data, list):
                for prog_data in programmes_data:
                    if isinstance(prog_data, dict):
                        ...
                        parsed_programmes.append(SettingsProgramme(
            ...
            parsed_faculties.append(
                SettingsFaculty(name=name, abbreviation=abbreviation, programmes=parsed_programmes)
            )
    ...
    def parse_data_settings(self, data_settings_val: dict[str, Any]) -> None:
        ...
        for key_str, value in data_settings_val.items():
            try:
                ...
                std_logger.warning(f"Unrecognized data setting key '{key_str}'. Skipping.")
                continue

            if setting_key == SheetSetting.DATA_ENTRY_COLS and isinstance(value, list):
                self.data_settings.data_entry_cols = [ColInfo(**col) for col in value if isinstance(col, dict)]
            elif setting_key == SheetSetting.COMPLETE_DATA_COLS and isinstance(value, list): # Not used in current code but parse if present
                self.data_settings.complete_data_cols = [ColInfo(**col) for col in value if isinstance(col, dict)]
            elif setting_key == SheetSetting.COMPLETE_DATA_NAME and isinstance(value, str):
            ...
            elif setting_key == SheetSetting.RAW_DATA_COL_ORDER and isinstance(value, list):
                self.data_settings.raw_data_col_order = [str(col_name) for col_name in value]
            elif setting_key == SheetSetting.FINAL_DATA_COL_ORDER and isinstance(value, list):
                self.data_settings.final_data_col_order = [str(col_name) for col_name in value]
                ...
                for colname, settings_dict in value.items():
                    if isinstance(settings_dict, dict):
                        ...
                        if "values" in settings_dict: field_spec["values"] = settings_dict["values"]
                        if "default" in settings_dict: field_spec["default"] = settings_dict["default"]
                        parsed_new_fields[str(colname)] = field_spec
                self.data_settings.new_fields = parsed_new_fields
            else:
                std_logger.warning(f"Skipping data setting '{key_str}' due to unexpected value type or unhandled case.")

    def parse_backup(self, backup_settings_val: dict[str, Any]) -> None:
        """Parses the 'backup' section of settings."""
        for key_str, value in backup_settings_val.items():
            try:
                setting_key = BackupSetting(value=key_str)
            except ValueError:
                std_logger.warning(f"Unrecognized backup setting key '{key_str}'. Skipping.")
                continue

            if setting_key == BackupSetting.BACKUP_ALL: self.backup_settings.backup_all = bool(value)
                ...
                for dir_key_str in value:
                    try:
                        ...
                        if dir_enum_val in self.dirs:
                            self.backup_settings.backup_dirs.append(self.dirs[dir_enum_val])
                        else: std_logger.warning(f"Directory key '{dir_key_str}' in backup_dirs not found in parsed self.dirs.")
                    except ValueError: std_logger.warning(f"Invalid DirSetting key '{dir_key_str}' in backup_dirs.")
            elif setting_key == BackupSetting.MAX_BACKUPS: self.backup_settings.max_backups = int(value)
            elif setting_key == BackupSetting.BACKUP_OVERVIEWS: self.backup_settings.backup_overviews = bool(value)
            else: std_logger.warning(f"Unhandled backup setting '{key_str}'.")
    ...
    def parse_directories(self, raw_dir_strs: dict[str, str]) -> None:
        ...
        for key_str, path_str in raw_dir_strs.items():
            try:
                ...
                std_logger.warning(f"Unrecognized directory key '{key_str}' in settings. Skipping.")
                continue
            ...
            try:
                ...
                std_logger.error(f"Error creating Directory object for key '{dir_key}' with path '{path_str}': {e}")
        ...
        if tmp_raw_full_data_path and DirSetting.RAW_COPYRIGHT_DATA in self.dirs:
            try:
                self.dirs[DirSetting.RAW_COPYRIGHT_DATA_FULL] = Directory(
                    path=self.dirs[DirSetting.RAW_COPYRIGHT_DATA].full / tmp_raw_full_data_path
                )
            except Exception as e:
                 std_logger.error(f"Error creating Directory for RAW_COPYRIGHT_DATA_FULL: {e}")
        elif tmp_raw_full_data_path:
            std_logger.warning("RAW_COPYRIGHT_DATA_FULL path found but RAW_COPYRIGHT_DATA base directory not yet parsed. RAW_COPYRIGHT_DATA_FULL might be incorrect.")
    ...
    def parse_files(self, raw_file_config: dict[str, Any]) -> None:
        ...
        if isinstance(script_data_folder_str, str):
            ...
            self.dirs[DirSetting.SCRIPT_DATA] = Directory(path=Path.cwd()) # Default if no folder specified
        ...
        if isinstance(files_dict, dict):
            for key_str, file_path_str in files_dict.items():
                ...
                except ValueError: std_logger.warning(f"Unrecognized file key '{key_str}' in files settings. Skipping.")
                except Exception as e: std_logger.error(f"Error creating File object for '{key_str}': {e}")
        ...
        if isinstance(subfolders_list, list):
            for subfolder_key_str in subfolders_list:
                try:
                    ...
                    self.dirs[dir_key] = Directory(path=base_path / str(subfolder_key_str))
                except ValueError: std_logger.warning(f"Unrecognized DirSetting key '{subfolder_key_str}' in subfolders. Skipping.")
                except Exception as e: std_logger.error(f"Error creating Directory for subfolder '{subfolder_key_str}': {e}")
    ...
    KEY_TO_PARSER_MAPPING: dict[str, Any] = {
        "university": parse_university,
        "data_settings": parse_data_settings,
        "directories": parse_directories,
        "files": parse_files,
        "unsorted": parse_unsorted, # Must be parsed after others that might use its values implicitly
        "backup": parse_backup,
    }
...
class SampleSettings:
    ...
    def load(self) -> None:
        ...
        try:
            ...
            std_logger.error(f"Sample settings file not found: {self.settings_file.path}.")
            self.raw_settings = {}
        except yaml.YAMLError as e:
            std_logger.error(f"Error parsing YAML from sample settings file {self.settings_file.path}: {e}")
            self.raw_settings = {}
        except Exception as e:
            std_logger.error(f"Unexpected error loading sample settings from {self.settings_file.path}: {e}")
            self.raw_settings = {}

    def parse_settings(self) -> None:
        """Parse all settings from the loaded YAML raw_settings."""
        if not isinstance(self.raw_settings, dict):
            std_logger.error(f"Failed to load sample settings as a dictionary. Raw settings type: {type(self.raw_settings)}")
            return
            ...
            if parser_method:
                try:
                    parser_method(self, value)
                     ...
                     std_logger.error(f"Error parsing sample settings section '{key}': {e}. Skipping.")
            else:
                std_logger.warning(f"[SampleSettings] Unrecognized key: '{key}'. Value will be set as direct attribute.")
                setattr(self, key, value)
    ...
    def parse_input(self, data_dict: dict[str, Any]) -> None:
        ...
        """
        self.input = {"file": data_dict.get("file", ""), "filters": data_dict.get("filters", [])}
    ...
    def parse_output(self, data_dict: dict[str, Any]) -> None:
        ...
        if not data_dict.get("file"):
             std_logger.warning('Sample settings "output" section is missing "file" key.')
...
def load_osiris_data() -> dict[str, dict[str, Any]] | None:
    ...
    try:
        ...
        if not hasattr(SETTINGS, 'files') or not isinstance(SETTINGS.files, dict) or FileSetting.OSIRIS_DATA_W_CONTACTS not in SETTINGS.files:
            std_logger.error("SETTINGS.files not properly initialized or OSIRIS_DATA_W_CONTACTS key missing.")
            return None
        ...
        std_logger.error( # Should be caught by osiris_file.exists above, but as fallback
            f"OSIRIS data file not found. OSIRIS data enrichment will not be possible."
        )
    except json.JSONDecodeError as e:
        std_logger.error(
             f"Error decoding JSON from OSIRIS data file: {e}"
        )
    except Exception as e:
        std_logger.error(f"Error loading OSIRIS data: {e}")
        std_logger.error(
            f"OSIRIS data file unreadable or other error. OSIRIS data enrichment will not be possible.\nPlease run the cli again with the refresh_osiris_data flag set to True to retrieve the required data."
        )
    return None
...
OSIRIS_DATA: dict[str, Any] | None = load_osiris_data() # OSIRIS_DATA value type more general
