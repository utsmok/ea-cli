...
def create_programme_overviews(
    ...
    if faculty not in COURSE_MAPPING:
        logger.warning(
            f"Faculty '{faculty}' not found in COURSE_MAPPING. Cannot create programme overviews."
        )
        return style_iter

    course_to_group: dict[str, str] = COURSE_MAPPING[faculty]
    programme_data_map: dict[str, list[pl.DataFrame]] = defaultdict(
        list
    )  # Store list of DFs per group to concat later
    ...
    for course_key_in_mapping, programme_group_name in course_to_group.items():
        ...
        programme_specific_data = all_faculty_data.filter(
            pl.col("department") == course_key_in_mapping
        )

        if programme_specific_data.is_empty():
            logger.debug(
                f"No data for course/dept '{course_key_in_mapping}' (group: {programme_group_name}) in faculty '{faculty}'."
            )
            continue
        ...
        if "pages_x_students" in programme_specific_data.columns:
            fine_series = (
                programme_specific_data["pages_x_students"]
            ...
            if "possible_fine" in programme_specific_data.columns:
                programme_specific_data = programme_specific_data.with_columns(
                    pl.coalesce(
                        pl.col("possible_fine").cast(pl.Float64, strict=False),
                        fine_series,
                    ).alias("possible_fine")
                )
            else:
                programme_specific_data = programme_specific_data.with_columns(
                    fine_series.alias("possible_fine")
                )
        else:
            logger.warning(
                f"'pages_x_students' column not found for {course_key_in_mapping}, cannot calculate possible_fine."
            )
            programme_specific_data = programme_specific_data.with_columns(
                pl.lit(None, dtype=pl.Float64).alias("possible_fine")
            )
        ...
        if "manual_classification" in programme_specific_data.columns:
            programme_specific_data = programme_specific_data.with_columns(
                infringement=pl.when(
                    ...
                    | pl.col("manual_classification").str.is_empty()
                ...
                .when(
                    ...
                    .str.to_lowercase()
                    .str.contains_any(
                        ["open access", "eigen materiaal", "overig", "deleted"]
                    )
                )  # Added "eigen materiaal"
                .then(pl.lit("no"))
                .when(
                    pl.col("manual_classification")
                    .str.to_lowercase()
            ...
            logger.warning(
                f"'manual_classification' column not found for {course_key_in_mapping}, cannot determine infringement status."
            )
            programme_specific_data = programme_specific_data.with_columns(
                pl.lit("undetermined").alias("infringement")
            )

        programme_data_map[programme_group_name].append(programme_specific_data)

    final_programme_dfs: dict[str, pl.DataFrame] = {
        group: pl.concat(dfs, how="diagonal_relaxed")
        for group, dfs in programme_data_map.items()
        if dfs
    }

    for group_name, group_df in final_programme_dfs.items():
        logger.info(
            f"Programme group '{group_name}' for faculty '{faculty}': {group_df.height} items."
        )
        ...
        backup_dir_for_programme_overviews = (
        ...
        for existing_file in Directory(
            programme_output_dir
        ).files:  # Uses utils.Directory
            if existing_file.name.startswith(
                f"{group_name}_total_overview_updated_"
            ) and existing_file.extension in [".xlsx", ".xls"]:
                try:
                    existing_file.move(
                        backup_dir_for_programme_overviews / existing_file.name
                    )
                    logger.debug(
                        f"Moved existing programme overview {existing_file.name} to backup."
                    )
                except Exception as e_move:
                    logger.warning(
                        f"Could not move existing programme overview {existing_file.name} to backup: {e_move}"
                    )
        ...
        logger.info(
            f"Saving programme overview for '{group_name}' ({faculty}) with {group_df.height} rows to {programme_file_obj.path.name}"
        )
        store_complete_data(programme_file_obj, group_df)  # Handles its own logging
        style_iter = finalize_sheet(
            programme_file_obj, group_df, style_iter
...
def create_faculty_overviews(
    ...
    """
    report_date_str = (
        file_date_str if file_date_str else datetime.now().strftime("%Y-%m-%d")
    )
    ...
    for faculty_abbr, faculty_df in faculty_data_map.items():
        if disable_writes:  # Skip file operations if writes are disabled
            aggregated_data_for_db_update.append(
                faculty_df
            )  # Still aggregate for potential DB update
            continue
        ...
        if faculty_df.is_empty():
            logger.info(
                f"No data for faculty {faculty_abbr}; skipping faculty overview sheet."
            )
            continue
        ...
        output_filename = (
            f"{faculty_abbr}_total_overview_updated_{report_date_str}.xlsx"
        )
        faculty_overview_file = File(faculty_dir / output_filename)  # utils.File

        logger.info(
            f"Saving faculty overview for {faculty_abbr} with {faculty_df.height} rows to {faculty_overview_file.path.name}."
        )
    ...
    if (
        aggregated_data_for_db_update and not disable_writes
    ):  # Only update DB if data exists and writes not disabled
        ...
        try:
            ...
            if loop.is_running():  # pragma: no cover
                ...
                logger.warning(
                    "update_db called from within a running event loop. Direct await might be needed."
                )
...
async def update_db(datalist: list[pl.DataFrame]) -> None:
    ...
    try:
        ...
        combined_df = pl.concat(datalist, how="diagonal_relaxed").unique(
            subset=["material_id"], keep="first", maintain_order=False
        )

        if combined_df.is_empty():
            logger.info(
                "Combined data for DB update is empty. No updates will be performed."
            )
            return

        logger.info(
            f"Updating/creating {combined_df.height} items in DB from overview data."
        )
        ...
        await update_copyright_items(
            combined_df, overwrite=False, update_relations=True
        )  # update_relations=True to be safe
