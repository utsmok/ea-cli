...
def create_programme_overviews(
    ...
    """
    course_to_group: dict[str, str] = settings.university_settings.course_mapping[faculty] # Use passed settings
    ...
    overview_fac_programme_dir = Directory(
        settings.dirs[DirSetting.OVERVIEWS_BACKUP].full / faculty / "per_programme" # Use passed settings
    )
    for groupname, df in data.items():
        for file in Directory(
            settings.dirs[DirSetting.FACULTIES_DIR].full / faculty / "per_programme" # Use passed settings
        ).files:
        ...
        store_complete_data(settings=settings, file=programme_file, data=df) # Pass settings
        style_iter = finalize_sheet(settings=settings, file=programme_file, data=df, style_iter=style_iter) # Pass settings
...
def create_faculty_overviews(
    ...
    for faculty, all_faculty_data in faculty_data.items():
        if faculty in settings.university_settings.course_mapping and not disable_writes: # Use passed settings
        ...
        if not disable_writes:
            ...
            store_complete_data(settings=settings, file=fac_file, data=all_faculty_data) # Pass settings
            style_iter = finalize_sheet(settings=settings, file=fac_file, data=all_faculty_data, style_iter=style_iter) # Pass settings
        data_to_update.append(all_faculty_data)

    asyncio.get_event_loop().run_until_complete(update_db(settings=settings, datalist=data_to_update)) # Pass settings
    return style_iter
