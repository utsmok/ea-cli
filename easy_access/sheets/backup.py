...
class Backupper:
    ...
    def backup_files(self) -> None:
        ...
        backup_location: Optional[Directory] = SETTINGS.backup_settings.backup_location
        max_backups: int = SETTINGS.backup_settings.max_backups

        if not dirs_to_backup:
            logger.warning("backup_all is True (or backup forced) but no backup_dirs specified in settings. Skipping backup.")
            return
        if not backup_location:
            logger.warning("backup_all is True (or backup forced) but no backup_location specified in settings. Skipping backup.")
            return
        if not backup_location.exists: # Ensure base backup location exists
            try:
                backup_location.create()
                logger.info(f"Created main backup location directory: {backup_location.full}")
            except Exception as e:
                logger.error(f"Failed to create main backup location {backup_location.full}: {e}. Skipping backup.")
                return

        if max_backups <= 0: # max_backups should be positive
            logger.warning(f"max_backups is {max_backups}, which is invalid or disables backup retention. Skipping backup.")
            return

        # Manage existing backups: delete oldest if count exceeds max_backups
        try:
            existing_backup_dirs = [d for d in backup_location.dirs if d.name.startswith("backup_")] # Filter for actual backup dirs
            while len(existing_backup_dirs) >= max_backups:
                if not existing_backup_dirs: break # Should not happen if len >= max_backups > 0
                oldest_backup = min(existing_backup_dirs, key=lambda d: d.created)
                logger.info(f"Max backups ({max_backups}) reached. Deleting oldest backup: {oldest_backup.full}")
                oldest_backup.delete()
                existing_backup_dirs = [d for d in backup_location.dirs if d.name.startswith("backup_")]
        except Exception as e:
            logger.error(f"Error managing existing backups: {e}. Proceeding with new backup if possible.")

        # Create new backup directory with a unique timestamped name
        timestamp_str = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        new_backup_dir_name = f"backup_{timestamp_str}"
        new_backup_dir = Directory(backup_location.full / new_backup_dir_name, create_dir=False)
        ...
        try:
            ...
            logger.error(f"Failed to create new backup directory {new_backup_dir.full}: {e}. Skipping backup.")
            return

        # Copy specified directories to the new backup location
        dir_names_to_backup_str = ", ".join([d.name for d in dirs_to_backup if d])
        logger.info(f"Starting backup of directories: [{dir_names_to_backup_str}] to {new_backup_dir.name}")
        for dir_to_backup in dirs_to_backup:
            if dir_to_backup and dir_to_backup.exists:
                try:
                    ...
                    dir_to_backup.copy(target_copy_path, overwrite=True) # Directory.copy handles its own logging
                except Exception as e_copy:
                    logger.error(f"Failed to copy directory {dir_to_backup.name} to backup: {e_copy}")
            else:
                logger.warning(f"Directory {str(dir_to_backup.name if dir_to_backup else 'N/A')} not found or invalid. Skipping its backup.")

        logger.info(f"Backup process completed. Data saved in {new_backup_dir.full}")

    def restore_backup(
        self,
        backup_source_location: Optional[Directory] = None, # Renamed from backup_dir for clarity
        ...
        """
        target_restore_dir: Optional[Directory] = SETTINGS.dirs.get(DirSetting.FACULTIES_DIR)
        if not target_restore_dir:
            logger.error("Target directory for restore ('faculties_dir') not configured in settings. Cannot restore.")
            return

        actual_backup_source_location: Optional[Directory] = backup_source_location or SETTINGS.backup_settings.backup_location
        if not actual_backup_source_location or not isinstance(actual_backup_source_location, Directory):
            logger.error(f"Invalid backup source location provided or configured: {actual_backup_source_location}")
            return
        if not actual_backup_source_location.exists:
            logger.error(f"Backup source location {actual_backup_source_location.full} does not exist. Cannot restore.")
            return

        available_backups: list[Directory] = [d for d in actual_backup_source_location.dirs if d.name.startswith("backup_")]
        if not available_backups:
            logger.warning(f"No backups found in {actual_backup_source_location.full}. Cannot restore.")
            return

        selected_backup_to_restore_from: Optional[Directory] = None
        if select == RestoreOptions.LATEST:
            selected_backup_to_restore_from = max(available_backups, key=lambda d: d.created)
        elif select == RestoreOptions.OLDEST:
            selected_backup_to_restore_from = min(available_backups, key=lambda d: d.created)
            ...
            for i, bk_dir in enumerate(available_backups):
                logger.info(f"  {i}: {bk_dir.name} (Created: {bk_dir.created.strftime('%Y-%m-%d %H:%M:%S')})")

            while True:
                try:
                    choice_str = input(f"  Select backup to restore by number (0-{len(available_backups) - 1}): ")
        ...
        if not selected_backup_to_restore_from or not selected_backup_to_restore_from.exists:
            logger.error(f"Selected backup ({selected_backup_to_restore_from.name if selected_backup_to_restore_from else 'None'}) is not valid or does not exist. Cannot restore.")
            return

        logger.info(f"Selected backup for restore: {selected_backup_to_restore_from.full}")
        ...
        try:
            if strategy == RestoreStrategy.REPLACE:
                if target_restore_dir.exists:
                    logger.info(f"Strategy '{strategy.value}': Deleting target directory {target_restore_dir.full} before restore.")
                ...
                for item_in_backup in selected_backup_to_restore_from.dirs: # Assuming backup stores backed up dirs as subdirs
                    target_path_for_item = target_restore_dir.full / item_in_backup.name
                    logger.info(f"Restoring {item_in_backup.name} to {target_path_for_item}...")
                ...
                backed_up_faculties_content = Directory(selected_backup_to_restore_from.full / target_restore_dir.name, create_dir=False)
                if backed_up_faculties_content.exists:
                    logger.info(f"Restoring content from {backed_up_faculties_content.full} to {target_restore_dir.full}")
                    backed_up_faculties_content.copy(target_restore_dir.full, overwrite=True)
                else:
                    logger.warning(f"Expected content for '{target_restore_dir.name}' not found in backup {selected_backup_to_restore_from.name}. Target may be empty.")


            elif strategy == RestoreStrategy.MERGE_PREFER_BACKUP:
                backed_up_faculties_content = Directory(selected_backup_to_restore_from.full / target_restore_dir.name, create_dir=False)
                if backed_up_faculties_content.exists:
                    logger.info(f"Merging (preferring backup) from {backed_up_faculties_content.full} into {target_restore_dir.full}")
                    backed_up_faculties_content.copy(target_restore_dir.full, overwrite=True) # overwrite=True prefers source (backup)
                else:
                     logger.warning(f"Expected content for '{target_restore_dir.name}' not found in backup. Nothing to merge.")

            elif strategy == RestoreStrategy.MERGE_PREFER_EXISTING:
                backed_up_faculties_content = Directory(selected_backup_to_restore_from.full / target_restore_dir.name, create_dir=False)
                if backed_up_faculties_content.exists:
                    logger.info(f"Merging (preferring existing) from {backed_up_faculties_content.full} into {target_restore_dir.full}")
                    backed_up_faculties_content.copy(target_restore_dir.full, overwrite=False) # overwrite=False prefers destination (existing)
                else:
                    logger.warning(f"Expected content for '{target_restore_dir.name}' not found in backup. Nothing to merge.")
            else: # Should be caught by Typer if strategy is an Enum
                logger.error(f"Unrecognized restore strategy: {strategy}. Cannot restore backup.")
                return
            logger.info(f"Backup restored to {target_restore_dir.full} using strategy: {strategy.value}")
