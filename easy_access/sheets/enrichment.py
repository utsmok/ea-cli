...
import re
import traceback
from collections.abc import Coroutine  # Corrected type hints
...
from easy_access.db.ingest import (
    load_base_data,  # Used by update_osiris_data at the end
)
from easy_access.settings import SETTINGS, FileSetting
from easy_access.utils import (
    determine_course_code,  # Assuming this is the correct location
)

logger = logging.getLogger(__name__)

# --- Configuration Candidates (to be moved to settings.yaml ideally) ---
OSIRIS_SEARCH_URL: str = (
    "https://utwente.osiris-student.nl/student/osiris/student/cursussen/zoeken"
)
OSIRIS_COURSE_DETAIL_URL_TEMPLATE: str = (
    "https://utwente.osiris-student.nl/student/osiris/owc/cursussen/{internal_id}"
)
PEOPLE_PAGE_OVERVIEW_URL_TEMPLATE: str = (
    "https://people.utwente.nl/overview?query={name}"
)
...
ACADEMIC_TITLES_TO_STRIP: list[str] = [
    "ing.",
    "dr.",
    "prof.",
    "ir.",
...
async def _get_data_from_osiris(
    ...
    str, dict[str, Any]
]:  # Return type reflects dict of course_code -> course_data
    """
    ...
    osiris_query_end: str = (
        ',"type":"phrase_prefix","fields":["cursus","cursus_korte_naam",'
        '"cursus_lange_naam"],"max_expansions":200}}]}}}'
    )

    current_year_filter = (
        f"{jaar}-{int(jaar) + 1}" if isinstance(jaar, int) else "2024-2025"
    )  # Default if jaar is empty string
    if jaar == "":  # Special case for retry to query all years
        final_query_body_str = (
            osiris_query_start.replace(
                '[{"terms":{"collegejaar":["2024-2025"]}}]', "[]"
            )
            + osiris_query_code_str
            + osiris_query_end
        )
    else:
        final_query_body_str = (
            osiris_query_start.replace('"2024-2025"', f'"{current_year_filter}"')
            + osiris_query_code_str
            + osiris_query_end
        )
    ...
    osiris_headers: dict[str, str] = {
        ...
        "pragma": "no-cache",
        "client_type": "web",
        "release_version": "c0d3b6a1d72bf1610166027c903b46fc10580f30",  # These might change
    ...
    try:
        async with semaphore:  # Limit concurrency
            response = await httpx_client.post(
                url=OSIRIS_SEARCH_URL, headers=osiris_headers, data=final_query_body_str
            )
            ...
            if not hits:
                if (
                    jaar and str(jaar) == "2018"
                ):  # Specific year to switch to all-years query
                    ...
                    logger.debug(
                        f"No Osiris data found for course code {input_number} (year: {current_year_filter})."
                    )
            else:
                if len(hits) > 1:
                    logger.info(
                        f"{len(hits)} Osiris search hits for code {input_number} (year {current_year_filter}). Processing all."
                    )

                for hit in hits:
                    raw_data = hit.get("_source", {})
                    course_code_from_hit = raw_data.get("cursus")
                    if not course_code_from_hit:
                        continue
                    ...
                    def _extract_names(items: Any) -> set[str]:
                        ...
                        if isinstance(items, str):
                            return {items.strip()} if items.strip() else set()
                        return set()

                    teachers_set: set[str] = set()
                    if isinstance(
                        raw_data.get("docenten"), list
                    ):  # Assuming 'docenten' is a list of dicts
                        for docente_item_list in raw_data.get("docenten", []):
                            ...
                            if isinstance(
                                docente_item_list, dict
                            ):  # e.g. {"naam": "Name"}
                                teachers_set.add(
                                    str(list(docente_item_list.values())[0])
                                )
                            elif isinstance(
                                docente_item_list, str
                            ):  # e.g. ["Name1", "Name2"]
                    ...
                    current_course_details: dict[str, Any] = {
                        ...
                        "faculty": raw_data.get("faculteit"),
                        "faculty_long": raw_data.get("faculteit_naam"),
                        "programme": raw_data.get("coordinerend_onderdeel_oms"),
                        "ec": raw_data.get("punten"),
                        "language": [
                            lang_item.get("voertaal_omschrijving")
                            for lang_item in raw_data.get("voertalen", [])
                            if lang_item
                        ],
                        "notes": raw_data.get("opmerking_cursus"),
                        "category": raw_data.get("categorie_omschrijving"),
                        "teachers": list(teachers_set),  # Store as list
                        "contacts": [],
                        "docenten": [],
                        "examinators": [],
                        "tutors": [],
                        "unknown_role": [],  # Initialize for detail page
                    }

                    # Fetch detailed contact page
                    internal_id = current_course_details.get("internal_id")
                    if internal_id:
                        detail_url = OSIRIS_COURSE_DETAIL_URL_TEMPLATE.format(
                            internal_id=internal_id
                        )
                        # Re-use headers, or define specific ones if needed
                        details_resp = await httpx_client.get(
                            url=detail_url, headers=osiris_headers
                        )
                        if details_resp.status_code == 200:
                            course_detail_json = details_resp.json()
                            for section in course_detail_json.get("items", []):
                                if (
                                    section.get("rubriek") == "rubriek-docenten"
                                ):  # Staff section
                                    for field_group in section.get("velden", []):
                                        role_description = field_group.get(
                                            "omschrijving"
                                        )
                                        for staff_entry_list in field_group.get(
                                            "waarde", []
                                        ):
                                            for (
                                                staff_member_field
                                            ) in staff_entry_list.get("velden", []):
                                                staff_name = staff_member_field.get(
                                                    "docent"
                                                )
                                                if staff_name:
                                                    if (
                                                    ...
                                                    elif role_description == "Docent":
                                                        current_course_details[
                                                            "docenten"
                                                        ].append(staff_name)
                                                    elif (
                                                        role_description == "Examinator"
                                                    ):
                                                        current_course_details[
                                                            "examinators"
                                                        ].append(staff_name)
                                                    elif role_description == "Tutor":
                                                        current_course_details[
                                                            "tutors"
                                                        ].append(staff_name)
                                                    else:
                            ...
                            logger.debug(
                                f"Failed to get course details from {detail_url}, status: {details_resp.status_code}"
                            )
                    processed_course_data[str(course_code_from_hit)] = (
                        current_course_details
                    )

    except httpx.HTTPStatusError as e_http:
        logger.warning(
            f"HTTP error fetching Osiris data for {input_number} (year {jaar}): {e_http.response.status_code} - {e_http.request.url}"
        )
    except json.JSONDecodeError as e_json:
        logger.warning(
            f"JSON decode error for Osiris data {input_number} (year {jaar}): {e_json}"
        )
    except Exception as e_generic:
        logger.error(
            f"Unexpected error in _get_data_from_osiris for {input_number} (year {jaar}): {e_generic}"
        )
    ...
    if should_retry_year:  # If a retry was triggered by previous logic
        logger.debug(
            f"Retrying _get_data_from_osiris for {input_number} with year parameter: '{jaar}'"
        )
...
async def _get_data_from_people_page(
    ...
    async with semaphore:
        search_url = PEOPLE_PAGE_OVERVIEW_URL_TEMPLATE.format(
            name=name
        )  # Use configured template
        try:
            search_response = await httpx_client.get(
                search_url, headers=people_page_headers
            )
            ...
            profile_link_pattern = (
                r'data-link="([^"]+)"'  # Assumes links are in data-link attribute
            )
            ...
            for match_suffix in matches[:10]:  # Check top 10 results
                ...
                if (
                    "business" in match_suffix or "/" not in match_suffix
                ):  # Example filter
                    continue
                ...
                name_from_suffix = (
                    match_suffix.split("/")[0] if "/" in match_suffix else match_suffix
                )
                current_ratio = Levenshtein.ratio(
                    compare_name_normalized,
                    _normalize_name_for_matching(name_from_suffix),
                )
            ...
            if (
                not best_match_link_suffix or best_ratio < 0.7
            ):  # Adjust threshold as needed
                logger.info(
                    f"Low match confidence ({best_ratio:.2f}) for '{name}'. Best guess: '{best_match_link_suffix}'. Skipping."
                )
                return None

            profile_url = (
                PEOPLE_PAGE_BASE_URL + best_match_link_suffix
            )  # Use configured base
            profile_response = await httpx_client.get(
                profile_url, headers=people_page_headers
            )
            ...
            main_name_tag = soup.find(
                "h1", class_="pageheader__title"
            )  # Example selector
            main_name_scraped = str(
                main_name_tag.find(string=True, recursive=False)
                if main_name_tag
                else ""
            ).strip()

            other_names_scraped: list[str] = []
            if main_name_tag and main_name_tag.find_all(
                "span"
            ):  # Example for other names/titles
                other_names_scraped = [
                    s.text.strip().replace("(", "").replace(")", "")
                    for s in main_name_tag.find_all("span")
                ]
            ...
            if email_tag:
                email_scraped = email_tag.get("href", "").replace("mailto:", "")
            ...
            org_section = soup.find(
                "div", class_="widget-linklist--smallicons"
            )  # This class was in original code
            if org_section:
                for org_item_tag in org_section.find_all(
                    "li"
                ):  # Assuming orgs are in list items
                    org_text_tag = org_item_tag.find(
            ...
            if education_tab:
                for link_tag in education_tab.find_all("a", href=True):
                    ...
                    if "utwente.osiris-student.nl" in href and " - " in link_text:
                        ...
                        "www.utwente.nl/" in href and link_text
                    ):  # Assuming these are programme links
                        programmes_scraped.append({"name": link_text, "url": href})

            return {
                "input_name": name,
                "main_name": main_name_scraped or normalized_input_name,
                "match_confidence": best_ratio,
                "other_names": other_names_scraped,
                "email": email_scraped,
                "orgs": orgs_scraped,
                "courses": courses_scraped,
                "programmes": programmes_scraped,
                "faculty": faculty_abbr_scraped,  # Use abbreviation
                "people_page_url": profile_url,
            }

        except httpx.HTTPStatusError as e_http_profile:
            logger.warning(
                f"HTTP error fetching profile for '{name}' (URL: {e_http_profile.request.url}): {e_http_profile.response.status_code}"
            )
...
async def update_osiris_data(
    ...
    if df_copyright_items.is_empty():
        logger.info(
            "No copyright items provided to update_osiris_data. Skipping enrichment."
        )
        return

    logger.info(
        f"Starting Osiris data update. Only retrieve missing: {only_retrieve_missing}"
    )

    # Determine unique course codes to look up from the input DataFrame
    unique_course_keys_df = df_copyright_items.select(
    ...
    for row in unique_course_keys_df.iter_rows(named=True):
        ...
        codes_from_row = determine_course_code(
            str(row.get("course_code", "")), str(row.get("course_name", ""))
        )
        lookup_course_codes.update(codes_from_row)

    if not lookup_course_codes:
        logger.info(
            "No valid course codes determined from input DataFrame. Skipping Osiris data fetch."
        )
        # Fall through to person data processing if that's independent or uses other criteria
    else:
        logger.info(
            f"Found {len(lookup_course_codes)} unique course codes to query in Osiris."
        )

    # Load existing Osiris data with contact details to check what's missing
    osiris_data_w_contacts_path: Path = SETTINGS.files[
        FileSetting.OSIRIS_DATA_W_CONTACTS
    ].path
    osiris_data_w_contacts_dict: dict[str, Any] = {}
    if osiris_data_w_contacts_path.exists():
        try:
            with open(osiris_data_w_contacts_path, encoding="utf-8") as f:
                osiris_data_w_contacts_dict = json.load(f)
        except (json.JSONDecodeError, OSError) as e:
            logger.warning(
                f"Could not load existing Osiris data with contacts from {osiris_data_w_contacts_path}: {e}"
            )
    ...
    if only_retrieve_missing:
        ...
        lookup_course_codes.difference_update(
            course_codes_already_retrieved
        )  # Remove already known
        logger.info(
            f"{len(lookup_course_codes)} course codes remain after filtering "
    ...
    if lookup_course_codes:  # Only fetch if there are codes to look up
        ...
        async with httpx.AsyncClient(
            timeout=60.0
        ) as client_osiris:  # Increased timeout
            osiris_tasks: list[Coroutine[Any, Any, dict[str, dict[str, Any]]]] = []
            for code_str in lookup_course_codes:
                if (
                    code_str.isdigit()
                ):  # Ensure it's a digit string before int conversion
                    osiris_tasks.append(
                        _get_data_from_osiris(
                            int(code_str), client_osiris, semaphore_osiris
                        )
                    )

            for result_dict in await asyncio.gather(
                *osiris_tasks, return_exceptions=True
            ):
                if isinstance(result_dict, dict):
                    all_fetched_osiris_courses.update(result_dict)
                elif isinstance(result_dict, Exception):
                    logger.warning(
                        f"An Osiris data fetching task failed: {result_dict}"
                    )
        logger.info(
            f"Fetched data for {len(all_fetched_osiris_courses)} new course codes from Osiris."
        )
    else:
        logger.info(
            "No new course codes to fetch from Osiris based on current settings."
        )
    ...
    if only_retrieve_missing:
        ...
        if osiris_data_path.exists():
            try:
                with open(osiris_data_path, encoding="utf-8") as f_curr:
                    current_osiris_data_no_contacts = json.load(f_curr)
                all_fetched_osiris_courses.update(current_osiris_data_no_contacts)
            except (json.JSONDecodeError, OSError) as e:
                logger.warning(
                    f"Could not load current osiris_data.json for merging: {e}"
                )

    if all_fetched_osiris_courses:  # Save if any data (new or merged)
        try:
            with open(
                SETTINGS.files[FileSetting.OSIRIS_DATA].path, "w", encoding="utf-8"
            ) as f_out:
                json.dump(
                    all_fetched_osiris_courses, f_out, indent=4, ensure_ascii=False
                )
            logger.info(
                f"Osiris course data saved to {SETTINGS.files[FileSetting.OSIRIS_DATA].path}"
            )
        except OSError as e:
    ...
    for course_detail in all_fetched_osiris_courses.values():
        for role_key in [
            ...
            if isinstance(persons_in_role, list):
                persons_to_retrieve_set.update(
                    str(p).strip()
                    for p in persons_in_role
                    if isinstance(p, str) and str(p).strip()
                )

    logger.info(
        f"Found {len(persons_to_retrieve_set)} unique person names from Osiris data for potential People Page lookup."
    )
    ...
    if person_data_path.exists():
        try:
            with open(person_data_path, encoding="utf-8") as f_person:
                current_person_data_list = json.load(f_person)
        except (json.JSONDecodeError, OSError) as e:
            logger.warning(
                f"Could not load existing person data from {person_data_path}: {e}"
            )

    if only_retrieve_missing:
        already_retrieved_persons: set[str] = {
            str(p.get("input_name", "")).strip()
            for p in current_person_data_list
            if p.get("input_name")
        }
        ...
        logger.info(
            f"{len(persons_to_retrieve_set)} person names remain for People Page lookup "
    ...
    if persons_to_retrieve_set:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client_people:
            ...
            for result_person_data in await asyncio.gather(
                *people_tasks, return_exceptions=True
            ):
                if isinstance(result_person_data, dict):
                    ...
                    logger.warning(
                        f"A People Page fetching task failed: {result_person_data}"
                    )
        logger.info(
            f"Fetched data for {len(newly_fetched_person_data)} new persons from People Pages."
        )
    ...
    if deduplicated_person_data_map:  # Check if there's anything to save
        try:
            with open(person_data_path, "w", encoding="utf-8") as f_person_out:
                json.dump(
                    list(deduplicated_person_data_map.values()),
                    f_person_out,
                    indent=4,
                    ensure_ascii=False,
                )
            logger.info(
                f"Person data saved to {person_data_path} ({len(deduplicated_person_data_map)} unique persons)."
            )
        except OSError as e:
    ...
    person_details_map: dict[str, dict[str, Any]] = {
        str(p.get("input_name", "")): p for p in deduplicated_person_data_map.values()
    }

    for course_code_str, course_info_dict in all_fetched_osiris_courses.items():
        if not course_info_dict:
            continue  # Skip if course info is empty for some reason
            ...
            for contact_name in contact_names_in_course:
                ...
                if person_detail:
                    enriched_contacts[contact_name_str] = {
                        ...
                        "first_name": person_detail.get(
                            "first_name"
                        ),  # Assuming first_name is directly available
                        "email": person_detail.get("email"),
                        "faculty": person_detail.get("faculty"),  # This is faculty abbr
                        "orgs": person_detail.get("orgs"),
                        "programmes": person_detail.get(
                            "programmes"
                        ),  # From people page scraping
                        "people_page": person_detail.get("people_page_url"),
                    }
                else:
                    logger.debug(
                        f"No detailed person data found for contact '{contact_name_str}' in course '{course_code_str}'."
                    )

        course_info_dict["contacts_detailed"] = (
            enriched_contacts  # Add new key for enriched data
        )
    ...
    if only_retrieve_missing:
        final_osiris_data_w_contacts.update(
            osiris_data_w_contacts_dict
        )  # Add old entries not re-fetched

    if final_osiris_data_w_contacts:
        try:
            with open(
                osiris_data_w_contacts_path, "w", encoding="utf-8"
            ) as f_contacts_out:
                json.dump(
                    final_osiris_data_w_contacts,
                    f_contacts_out,
                    indent=4,
                    ensure_ascii=False,
                )
            logger.info(
                f"Osiris data with enriched contacts saved to {osiris_data_w_contacts_path}"
            )
        except OSError as e:
    ...
    logger.info(
        "Running load_base_data to ensure all new entities from enrichment are in DB."
    )
