    ...
    try:
        if not file:
            ...
            if not raw_data_dir or not raw_data_dir.exists:
                logger.warning(
                    f"Raw copyright data directory not found or configured. Searched at: {raw_data_dir.full if raw_data_dir else 'N/A'}"
                )
                raise typer.Exit(code=1)

            excel_files = [
                f
                for f in raw_data_dir.files
        ...
        processed_df = (
            ...
            )  # Cast all to string first for safety
            .rename(
                lambda c: str(c)
                .replace(" ", "_")
                .replace("#", "count_")
            ...
            .with_columns(
                ...
                pl.col("last_change")
                .str.replace(r"^- ভারতবর্ষ$", None)  # Specific cleaning for placeholder
                .str.strip_chars()
        ...
        logger.info(
            f"{len(processed_df)} items remaining after initial filtering from {file.name}."
        )
        return file_date_str, processed_df

    except FileNotFoundError:  # Should be caught by initial checks, but as safeguard
        logger.error(
            f"Critical: Copyright data file/directory not found. Path: {SETTINGS.dirs.get(DirSetting.RAW_COPYRIGHT_DATA, 'N/A')}"
        )
        raise typer.Exit(code=1)
        ...
        ValueError
    ) as e:  # e.g., max() on empty sequence if no files found and not handled
        logger.error(
            f"ValueError during copyright export processing (e.g. no files found): {e}"
        )
        raise typer.Exit(code=1)
    except Exception as e:  # Catch any other Polars or unexpected errors
        logger.error(
            f"Unexpected error in read_copyright_export for file {file.name if file else 'N/A'}: {e}"
        )
...
class DataEntrySheet:
    ...
    word_wrap_style: Alignment = field(
        default_factory=lambda: NamedStyle(
            name="wordwrap_dynamic", alignment=Alignment(wrapText=True)
        )
    )
    ...
    def __post_init__(self) -> None:
        ...
        self.sheet = self.workbook.create_sheet(
            self.sheet_name, index=1
        )  # Create after 'Complete Data'
    ...
    def add_data(self, data: pl.DataFrame) -> None:
        ...
        if data.is_empty():
            logger.warning(
                f"Data provided to DataEntrySheet '{self.sheet_name}' is empty."
            )
            # Still create headers
            for col_idx, col_info in enumerate(self.cols):
                self.sheet.cell(1, col_idx + 1).value = (
                    col_info.new_name or col_info.name
                )
            self.max_row = 0
        else:
        ...
        for col_info in self.cols:
            ...
            if col_info.is_new:
                ...
                if col_info.default_val != "":
                    column_data_list = series_data.apply(
                        lambda x: col_info.default_val
                ...
                logger.warning(
                    f"Column '{col_info.name}' not found in DataFrame for sheet '{self.sheet_name}'. Filling with empty strings."
                )
                column_data_list = [""] * self.max_row

            # Write data cells
            for row_idx, cell_val in enumerate(
                column_data_list, start=2
                ...
                cell_display_value = str(
                    current_cell.value if current_cell.value is not None else ""
                )
    ...
    def _format_columns(self) -> None:
        ...
        for col_idx, col_info in enumerate(self.cols, start=1):
            ...
            if (
                col_info.has_dropdown and self.max_row > 0
            ):  # Only add validation if there's data
                dv = openpyxl.worksheet.datavalidation.DataValidation(
                    type="list",
                    formula1=f'"{col_info.dropdown_options}"',
                    allowBlank=True,
                )
                ...
                dv.add(
                    f"{col_letter}2:{col_letter}{self.max_row + 1}"
                )  # Apply to data rows

            # Column width adjustment heuristic
            dim = self.sheet.column_dimensions[col_letter]
            if col_info.max_width > 40 and (
                col_info.count_max_width_over_40 > 5
                or col_info.count_max_width_over_40 > self.max_row * 0.2
            ):
                dim.width = 40
                ...
                if self.max_row > 0:
                    for row_num in range(2, self.max_row + 2):
                        self.sheet.cell(
                            ...
                            self.word_wrap_style.alignment
                        )  # Apply alignment part
            else:
                dim.width = max(
                    col_info.max_width + 2,
                    len(col_info.new_name or col_info.name) + 2,
                    10,
                )  # Min width 10
    ...
    def create_table(self) -> None:
        ...
        max_col_letter = openpyxl.utils.get_column_letter(
            len(self.cols) if self.cols else 1
        )
        ...
        table = ExcelTable(
            displayName=self.sheet_name.replace(" ", "_").replace("-", "_")[
                :30
            ],  # Sanitize name
...
def finalize_sheet(file: File, data: pl.DataFrame, style_iter: int) -> int:
    ...
    if complete_data_sheet_name not in wb.sheetnames:
        if len(wb.sheetnames) == 1:  # If only one sheet, assume it's the complete data
            ...
            logger.warning(
                f"Sheet '{complete_data_sheet_name}' not found in {file.name}. Active sheet titled '{wb.active.title}'."
            )
    ...
    tabstyle = TableStyleInfo(
        name=f"TableStyleMedium{table_style_number}",
        showRowStripes=True,
    )
    ...
    data_for_entry_sheet = data.unique(
        subset=["material_id"], keep="first", maintain_order=True
    )
    ...
    if (
        "overview" in file.path.stem.lower()
    ):  # Check if "overview" is in the filename stem
        logger.debug(f"Processing LLM classification for overview file: {file.name}")
        llm_data = enrich_with_llm_classifications(
            data_for_entry_sheet
        )  # Uses data from data entry
        if not llm_data.is_empty():
            ...
            llm_sheet_path = (
                file.path.parent / f"{file.path.stem}_llm_data.xlsx"
            )  # Storing in a separate file now

            try:
                llm_data.write_excel(
                    workbook=str(
                        llm_sheet_path
                    ),  # write_excel expects string path for new workbook
                ...
                logger.info(
                    f"Stored LLM classification data to separate sheet: {llm_sheet_path.name}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to write LLM classification sheet for {file.name}: {e}"
                )
        else:
...
def store_complete_data(output_file_path: Path | File, data: pl.DataFrame) -> None:
    ...
    path_obj: Path = (
        ...
        if isinstance(output_file_path, File)
    ...
    if path_obj.exists() and path_obj.stat().st_size > 0:
        logger.info(
            f"File {path_obj} already exists and is not empty. It will be overwritten."
        )
    ...
    if (
        not cols_to_select
    ):  # Fallback to all columns if none of the specified are present
        logger.warning(
            "No columns from final_data_col_order found in data. Writing all columns."
        )
        cols_to_select = data.columns

    processed_data = data.select(cols_to_select).unique(
        subset=["material_id"], keep="first", maintain_order=True
    )
...
def read_export_sheets() -> pl.DataFrame:
    ...
    if not export_dir or not export_dir.exists:
        logger.warning(
            f"Export directory not found or configured: {export_dir.full if export_dir else 'N/A'}"
        )
        return pl.DataFrame()
    ...
    for file_obj in export_dir.files:  # Assumes .files gives File objects
        try:
            ...
            logger.warning(
                f"Could not read or process export file {file_obj.name}: {e}"
            )
...
def create_export_sheet(
    ...
    TODAY_STR: str = datetime.now().strftime(
        "%Y-%m-%d_%H-%M-%S"
    ...
    if export_data_subset.is_empty():
        logger.warning(
            f"No items with workflow_status 'Done' found for '{faculty_name or 'ALL'}'. Skipping export."
        )
        return
    ...
    Directory(current_export_dir_path).mkdir(
        parents=True, exist_ok=True
    )  # Ensure export dir exists

    file_prefix_str = f"utwente_{faculty_name or 'GLOBAL'}_{TODAY_STR}_{export_data_subset.height}_items"

    main_export_file_path = (
        current_export_dir_path / f"{file_prefix_str}_copyright_import.xlsx"
    )
    details_export_file_path = (
        current_export_dir_path
        / f"{file_prefix_str}_copyright_import_full_details.xlsx"
    )

    logger.info(
        f"Exporting {export_data_subset.height} items for '{faculty_name or 'GLOBAL'}' to {current_export_dir_path}"
    )

    # Create the main, minimal export sheet
    try:
        # Ensure only existing columns from EXPORT_COL_NAMES are selected
        cols_for_main_export = [
            col for col in EXPORT_COL_NAMES if col in export_data_subset.columns
        ]
        export_data_subset.select(cols_for_main_export).write_excel(
            str(main_export_file_path)
        )
        logger.info(f"Main export sheet saved to: {main_export_file_path.name}")
    except Exception as e:
        logger.error(
            f"Failed to write main export sheet {main_export_file_path.name}: {e}"
        )

    # Store the sheet with full details using store_complete_data
    try:
        store_complete_data(
            file=details_export_file_path, data=export_data_subset
        )  # store_complete_data handles its own logging
        logger.info(
            f"Full details export sheet saved to: {details_export_file_path.name}"
        )
    except Exception as e:
        logger.error(
            f"Failed to write full details export sheet {details_export_file_path.name}: {e}"
        )
...
def retrieve_all_classifications() -> pl.DataFrame:
    ...
    if not classifications_dir or not classifications_dir.exists:
        logger.warning(
            f"Classifications directory not found: {classifications_dir.full if classifications_dir else 'N/A'}"
        )
        return pl.DataFrame()

    all_files_in_dir: list[File] = (
        classifications_dir.files_r
    ...
    all_json_files: list[File] = [
        ...
        if f.extension == ".json"
        and "_" not in f.name
        and f.name.rstrip(".json").isdigit()
    ]
    ...
    for json_file in all_json_files:
        ...
        try:
            ...
            if current_mat_id in processed_mat_ids:
                continue  # Already processed (e.g. via a replacement)
    ...
    temp_data_by_id: dict[int, dict[str, Any]] = {
        item["material_id"]: item for item in classification_data_list
    }
    ...
    for old_id, new_id in all_replacement_files_info.items():
        if (
            old_id in temp_data_by_id
        ):  # Data for old_id exists, maybe it's also a replacement target
            logger.debug(
                f"Data for {old_id} already processed directly. Replacement by {new_id} might be redundant or a chain."
            )
        ...
        # If old_id's data is NOT directly present, but it's replaced by new_id for which we DO have data:
            ...
            replacement_data_content["material_id"] = (
                old_id  # Attribute this data to the old_id
            )
            final_data_for_df.append(replacement_data_content)
            logger.info(
                f"Applied replacement: Data for {new_id} is now also used for {old_id}."
            )
        elif new_id not in temp_data_by_id:
            logger.warning(
                f"Replacement for {old_id} by {new_id} specified, but no data found for {new_id}."
            )
