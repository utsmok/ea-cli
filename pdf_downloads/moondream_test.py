...
def _(Image, Path, pymupdf):
    def get_pdfs_as_img(
        dir: str = ".", max_items: int = 50, imgs_per_doc: int = 1
    ) -> dict[str, list[Image]]:
    ...
    def enrich_with_moondream():
        ...
        for filename, data in pdfs_as_imgs.items():
            ...
            for img_num, img in enumerate(data["images"]):
                ...
                if img_num == 0:
                    try:
                        data["type"] = model.query(
                            img, "what type of document is this?"
                        )
                        print(data["type"])
                    except Exception:
                        print(f"failed to get type, skipping")
                    try:
                        data["authors"] = model.query(
                            img, "who are the authors of this document?"
                        )
                        print(data["authors"])
                    except Exception:
