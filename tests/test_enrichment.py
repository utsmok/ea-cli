...
async def test_persist_courses_new(setup_test_db):
    ...
    with (
        patch(
            "easy_access.enrichment.osiris.Course.filter", return_value=QuerySetMock([])
        ),
        patch(
            "easy_access.enrichment.osiris.Course.create", new=AsyncMock()
        ) as mock_create,
    ):
...
async def test_persist_persons_new(setup_test_db):
    ...
    with (
        patch(
            "easy_access.enrichment.osiris.Person.filter", return_value=QuerySetMock([])
        ),
        patch(
            "easy_access.enrichment.osiris.Person.create", new=AsyncMock()
        ) as mock_create,
    ):
