...
async def test_select_missing_or_stale_courses_ttl_none():
    ...
    with (
        patch(
            "easy_access.enrichment.osiris.Course.filter",
            return_value=QuerySetMock([mock_course]),
        ),
        patch(
            "easy_access.enrichment.osiris.MissingCourse.filter",
            return_value=QuerySetMock(tracked_missing),
        ),
    ):
        to_fetch = await osiris.select_missing_or_stale_courses(
            None, course_codes, ttl_days=None
        )
...
async def test_select_missing_or_stale_courses_with_ttl():
    ...
    with (
        patch(
            "easy_access.enrichment.osiris.Course.filter",
            return_value=QuerySetMock([stale_course, fresh_course]),
        ),
        patch(
            "easy_access.enrichment.osiris.MissingCourse.filter",
            return_value=QuerySetMock(tracked_missing),
        ...
        to_fetch = await osiris.select_missing_or_stale_courses(
            None, course_codes, ttl_days=ttl
        )
...
async def test_select_missing_or_stale_persons_ttl_none():
    ...
    with patch(
        "easy_access.enrichment.osiris.Person.filter",
        return_value=QuerySetMock([existing_person]),
    ):
        to_fetch = await osiris.select_missing_or_stale_persons(
            None, names, ttl_days=None
        )
...
async def test_select_missing_or_stale_persons_with_ttl():
    ...
    with patch(
        "easy_access.enrichment.osiris.Person.filter",
        return_value=QuerySetMock([alice, eve]),
    ):
        to_fetch = await osiris.select_missing_or_stale_persons(
            None, names, ttl_days=ttl
        )
...
async def test_fetch_and_parse_courses_records_missing_and_returns_found():
    ...
    with (
        patch(
            "easy_access.enrichment.osiris.fetch_course_data",
            new=AsyncMock(side_effect=fake_fetch),
        ),
        patch(
            "easy_access.enrichment.osiris.MissingCourse.get_or_none",
            new=AsyncMock(return_value=None),
        ),
        patch(
            "easy_access.enrichment.osiris.MissingCourse.create", new=AsyncMock()
        ) as mock_create,
        patch(
            "easy_access.enrichment.osiris.MissingCourse.filter",
        ...
        results = await osiris.fetch_and_parse_courses(None, {1, 2}, max_concurrent=2)
