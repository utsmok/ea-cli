...
async def compare_pdfs():
    ...
    all_pdfs = await PDF.all().prefetch_related('replace_with').values('material_id', 'author', 'title', 'replace_with__material_id')
    all_pdfs = pl.from_dicts(all_pdfs)
    pdfs_with_duplicates = all_pdfs.filter(pl.col('replace_with__material_id').is_not_null()).to_dicts()
    print(f'Got {len(pdfs_with_duplicates)}/{len(all_pdfs)} pdfs are duplicates')

    for dupe in pdfs_with_duplicates:
        original = all_pdfs.filter(pl.col('material_id') == dupe.get('replace_with__material_id')).to_dicts()[0]
        compare = {
        ...
        dupe_copyright_item = await CopyrightItem.get_or_none(material_id=dupe.get('material_id'))

        original_copyright_item = await CopyrightItem.get_or_none(material_id=original.get('material_id'))
        if dupe_copyright_item and original_copyright_item:
            compare['ci_author'] = (dupe_copyright_item.author, original_copyright_item.author)
            compare['ci_title'] = (dupe_copyright_item.title, original_copyright_item.title)
            compare['ci_filename'] = (dupe_copyright_item.filename, original_copyright_item.filename)
            compare['ci_course_name'] = (dupe_copyright_item.course_name, original_copyright_item.course_name)
            compare['ci_man_class'] = (dupe_copyright_item.manual_classification, original_copyright_item.manual_classification)
